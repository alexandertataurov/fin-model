...
function fixStoryFile(filePath) {
  ...
  try {
    ...
    // Fix malformed JSX closing tags that were created by the previous script
    const originalContent = content;

    // Fix misplaced closing tags like </StoryCard></div> to </div></StoryCard>
    content = content.replace(/<\/StoryCard><\/div>/g, '</div></StoryCard>');
    content = content.replace(/<\/StoryCard><\/h3>/g, '</h3></StoryCard>');
    content = content.replace(/<\/StoryCard><\/p>/g, '</p></StoryCard>');

    // Fix missing closing tags for StoryCard components
...
// Main execution
async function main() {
  console.log('🔧 Fixing Storybook syntax errors (v2)...\n');
