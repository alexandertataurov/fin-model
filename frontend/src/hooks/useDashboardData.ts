...
export const useDashboardRefresh = () => {
  ...
  const refreshDashboard = async (dashboardType?: 'cash-flow' | 'pl' | 'all') => {
...
export const useDashboardPrefetch = () => {
  ...
  const getCachedData = <T>(type: 'CASH_FLOW' | 'PL', period: DashboardPeriod, fileId?: number): T | undefined => {
    return DashboardCacheManager.getCachedData<T>(queryClient, type, period, fileId);
  };

  const isDataStale = (type: 'CASH_FLOW' | 'PL', period: DashboardPeriod, fileId?: number): boolean => {
