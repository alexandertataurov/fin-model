          ...
          switch (message.type) {
            case 'SYSTEM_STATS':
              ...
              if (
                ...
                message.timestamp > systemStats.lastUpdated
              ) {
            ...
            case 'USER_ACTIVITY':
              if (
                ...
                message.timestamp > userActivity.lastUpdated
              ) {
                useAdminStore.setState(state => ({
                  userActivity: {
                    ...state.userActivity,
            ...
            case 'SYSTEM_METRICS':
              if (
                ...
                message.timestamp > systemMetrics.lastUpdated
              ) {
                useAdminStore.setState(state => ({
                  systemMetrics: {
                    ...state.systemMetrics,
              ...
              if (!logs.lastUpdated || message.timestamp > logs.lastUpdated) {
                useAdminStore.setState(state => ({
                  logs: {
                    ...
                    total:
                      message.data.total || message.data.items?.length || 0,
                    lastUpdated: message.timestamp,
                  },
                }));
              }
