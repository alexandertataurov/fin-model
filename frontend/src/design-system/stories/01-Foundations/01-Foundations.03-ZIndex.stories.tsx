...
const meta: Meta = {
  title: 'Design System/1 - Foundations/3 - Z-Index',
  parameters: {
    docs: {
      autodocs: true,
      page: () => (
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <Card className="p-6 overflow-hidden">
          <h4 style={applyTypographyStyle('subtitle')} className="text-muted-foreground mb-4 break-words">Navigation with Dropdown (z-dropdown: {tokens.zIndex.dropdown})</h4>
          <div className="flex items-center space-x-4 flex-wrap">
            <div className="bg-primary/10 px-4 py-2 rounded-md text-primary font-medium text-sm">Logo</div>
            <div className="bg-muted px-4 py-2 rounded-md text-sm">Home</div>
            <div className="relative">
              <div className="bg-muted px-4 py-2 rounded-md cursor-pointer hover:bg-muted/80 text-sm">Products ▼</div>
              <div
              ...
              >
                <div className="px-2 py-1 hover:bg-muted rounded text-sm break-words">Product A</div>
                <div className="px-2 py-1 hover:bg-muted rounded text-sm break-words">Product B</div>
                <div className="px-2 py-1 hover:bg-muted rounded text-sm break-words">Product C</div>
              </div>
            </div>
          </div>
        </Card>
        ...
        {/* Sticky header */}
        <Card className="p-6 overflow-hidden">
          <h4 style={applyTypographyStyle('subtitle')} className="text-muted-foreground mb-4 break-words">Sticky Header (z-sticky: {tokens.zIndex.sticky})</h4>
          <div className="space-y-4">
            <div
              className="bg-background border-b px-6 py-3 flex items-center justify-between"
              style={{ zIndex: tokens.zIndex.sticky }}
            >
              <div className="font-semibold text-sm break-words">Sticky Header</div>
        ...
        {/* Modal overlay */}
        <Card className="p-6 overflow-hidden">
          <h4 style={applyTypographyStyle('subtitle')} className="text-muted-foreground mb-4 break-words">Modal with Backdrop (z-modal: {tokens.zIndex.modal})</h4>
          <div className="relative h-40 bg-muted/20 rounded-md overflow-hidden">
            ...
            >
              <div className="bg-background border rounded-lg p-6 max-w-sm mx-4">
                ...
                <p className="text-sm text-muted-foreground mb-4 break-words">This modal appears above the backdrop.</p>
        ...
        {/* Tooltip */}
        <Card className="p-6 overflow-hidden">
          <h4 style={applyTypographyStyle('subtitle')} className="text-muted-foreground mb-4 break-words">Tooltip (z-tooltip: {tokens.zIndex.tooltip})</h4>
          <div className="flex items-center space-x-4 flex-wrap">
            <div className="relative">
              <div className="bg-primary px-4 py-2 rounded-md text-primary-foreground cursor-help text-sm">Hover me</div>
              <div
            ...
            <div className="text-sm text-muted-foreground break-words">Tooltips appear above all other elements</div>
          </div>
        </Card>

        {/* Fixed sidebar */}
        <Card className="p-6 overflow-hidden">
          <h4 style={applyTypographyStyle('subtitle')} className="text-muted-foreground mb-4 break-words">Fixed Sidebar (z-fixed: {tokens.zIndex.fixed})</h4>
          <div className="relative h-40 bg-muted/20 rounded-md overflow-hidden">
            <div
            ...
            >
              <div className="p-4">
                <div className="font-semibold mb-4 text-sm break-words">Sidebar</div>
                <div className="space-y-2">
                  <div className="px-2 py-1 hover:bg-muted rounded text-sm break-words">Dashboard</div>
                  <div className="px-2 py-1 hover:bg-muted rounded text-sm break-words">Analytics</div>
                  <div className="px-2 py-1 hover:bg-muted rounded text-sm break-words">Settings</div>
                </div>
              </div>
            </div>
            <div className="ml-48 p-4">
              <div className="text-sm text-muted-foreground break-words">Main content area</div>
            </div>
          </div>
        </Card>
        ...
        {/* Popover menu */}
        <Card className="p-6 overflow-hidden">
          <h4 style={applyTypographyStyle('subtitle')} className="text-muted-foreground mb-4 break-words">Popover Menu (z-popover: {tokens.zIndex.popover})</h4>
          <div className="flex items-center space-x-4 flex-wrap">
            <div className="relative">
              <div className="bg-muted px-4 py-2 rounded-md cursor-pointer hover:bg-muted/80 text-sm">Actions ▼</div>
              <div
            ...
            <div className="text-sm text-muted-foreground break-words">Context menus appear above dropdowns</div>
          </div>
        </Card>
      </div>
    </div>
...
export const Guidelines: Story = {
  render: () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-3">
          <h4 style={applyTypographyStyle('subtitle')} className="text-foreground break-words">Semantic Tokens</h4>
          <div className="space-y-2 text-sm">
            {Object.entries(tokens.zIndex)
              .filter(([name, z]) => typeof z === 'string' && !isNaN(Number(z)) && Number(z) >= 1000)
              .map(([name, z]) => (
                <div key={name} className="flex justify-between items-center">
                  <span className="text-muted-foreground capitalize break-words">{name.replace(/([A-Z])/g, ' $1').trim()}</span>
        ...
        <div className="space-y-3">
          <h4 style={applyTypographyStyle('subtitle')} className="text-foreground break-words">Numeric Tokens</h4>
          <div className="space-y-2 text-sm">
            {Object.entries(tokens.zIndex)
              .filter(([name, z]) => typeof z === 'string' && !isNaN(Number(z)) && Number(z) <= 50)
              .map(([name, z]) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-3">
          <h4 style={applyTypographyStyle('subtitle')} className="text-foreground break-words">Best Practices</h4>
          <ul className="space-y-2 text-sm text-muted-foreground">
            <li className="flex items-start space-x-2">
              <span className="text-primary mt-1">•</span>
              <span className="break-words">Use semantic tokens for common UI patterns</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-primary mt-1">•</span>
              <span className="break-words">Avoid arbitrary z-index values</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-primary mt-1">•</span>
              <span className="break-words">Keep z-index values as low as possible</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-primary mt-1">•</span>
              <span className="break-words">Consider stacking context when nesting elements</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-primary mt-1">•</span>
              <span className="break-words">Test z-index behavior across different browsers</span>
            </li>
          </ul>
        </div>

        <div className="space-y-3">
          <h4 style={applyTypographyStyle('subtitle')} className="text-foreground break-words">Common Pitfalls</h4>
          <ul className="space-y-2 text-sm text-muted-foreground">
            <li className="flex items-start space-x-2">
              <span className="text-destructive mt-1">•</span>
              <span className="break-words">Using values above {tokens.zIndex.tooltip} unless critical</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-destructive mt-1">•</span>
              <span className="break-words">Creating unnecessary stacking contexts</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-destructive mt-1">•</span>
              <span className="break-words">Forgetting about transform and opacity effects</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-destructive mt-1">•</span>
              <span className="break-words">Not considering mobile vs desktop behavior</span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-destructive mt-1">•</span>
              <span className="break-words">Ignoring accessibility implications</span>
            </li>
          </ul>
        </div>
      </div>
  ...
  render: () => (
    <div className="space-y-8">
      ...
      <div className="relative h-96 bg-gradient-to-b from-blue-900 to-blue-700 rounded-xl overflow-hidden border">
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 w-8 h-8 bg-red-500 rounded-full" style={{ zIndex: 20 }}></div>

        {/* UI elements layer (z=30) */}
        <div className="absolute top-4 left-4 bg-black/50 text-white px-3 py-1 rounded text-sm" style={{ zIndex: 30 }}>
          Score: 12,450
        </div>
        <div className="absolute top-4 right-4 bg-black/50 text-white px-3 py-1 rounded text-sm" style={{ zIndex: 30 }}>
          Level: 8
        </div>

        {/* Floating menu (z=dropdown) */}
        <div className="absolute top-16 right-4 bg-white border rounded-lg shadow-lg p-3" style={{ zIndex: tokens.zIndex.dropdown }}>
          <div className="text-sm font-medium text-gray-900 mb-2 break-words">Game Menu</div>
          <div className="space-y-1">
            <div className="px-2 py-1 hover:bg-gray-100 rounded text-sm cursor-pointer break-words">Resume</div>
            <div className="px-2 py-1 hover:bg-gray-100 rounded text-sm cursor-pointer break-words">Settings</div>
            <div className="px-2 py-1 hover:bg-gray-100 rounded text-sm cursor-pointer break-words">Save Game</div>
            <div className="px-2 py-1 hover:bg-red-100 rounded text-sm cursor-pointer text-red-600 break-words">Quit</div>
          </div>
        </div>

        {/* Notification (z=popover) */}
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-yellow-100 border border-yellow-300 rounded-lg p-3 shadow-lg" style={{ zIndex: tokens.zIndex.popover }}>
          <div className="text-sm font-medium text-yellow-800 break-words">Achievement Unlocked!</div>
          <div className="text-xs text-yellow-700 break-words">Complete 10 levels</div>
        </div>

        {/* Pause overlay (z=modal) */}
        <div className="absolute inset-0 bg-black/70 flex items-center justify-center" style={{ zIndex: tokens.zIndex.modal }}>
          <div className="bg-white rounded-lg p-6 max-w-sm mx-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-4 break-words">Game Paused</h3>
            <div className="space-y-2">
              <button className="w-full px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">Resume</button>
              <button className="w-full px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 text-sm">Settings</button>
              <button className="w-full px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 text-sm">Quit Game</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="relative h-80 bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="absolute inset-0 p-6" style={{ zIndex: 20 }}>
          <div className="h-full flex items-end justify-around">
            ...
            <div className="w-8 bg-blue-700 rounded-t" style={{ height: '45%' }}></div>
            <div className="w-8 bg-blue-800 rounded-t" style={{ height: '90%' }}></div>
            <div className="w-8 bg-blue-900 rounded-t" style={{ height: '70%' }}></div>
          </div>
        </div>
        ...
        <div className="absolute inset-0 p-6" style={{ zIndex: 30 }}>
          <div className="h-full flex items-end justify-around">
            <div className="text-xs text-gray-600 text-center break-words">Jan</div>
            <div className="text-xs text-gray-600 text-center break-words">Feb</div>
            <div className="text-xs text-gray-600 text-center break-words">Mar</div>
            <div className="text-xs text-gray-600 text-center break-words">Apr</div>
            <div className="text-xs text-gray-600 text-center break-words">May</div>
          </div>
        </div>

        {/* Hover tooltip (z=tooltip) */}
        <div className="absolute top-1/4 left-1/3 bg-gray-900 text-white px-2 py-1 rounded text-xs shadow-lg" style={{ zIndex: tokens.zIndex.tooltip }}>
          <div className="break-words">Feb: 8,240 units</div>
          <div className="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-gray-900"></div>
        </div>

        {/* Legend overlay (z=dropdown) */}
        <div className="absolute top-4 right-4 bg-white border rounded-lg shadow-lg p-3" style={{ zIndex: tokens.zIndex.dropdown }}>
          <div className="text-sm font-medium text-gray-900 mb-2 break-words">Legend</div>
          <div className="space-y-1">
            <div className="flex items-center space-x-2">
        ...
        <div className="absolute bottom-4 left-4 bg-white border rounded-lg shadow-lg p-2" style={{ zIndex: tokens.zIndex.popover }}>
          <div className="flex space-x-2">
            <button className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs hover:bg-blue-200 break-words">Bar</button>
            <button className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs hover:bg-gray-200 break-words">Line</button>
            <button className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs hover:bg-gray-200 break-words">Area</button>
          </div>
        </div>

        {/* Fullscreen modal (z=modal) */}
        <div className="absolute inset-0 bg-black/80 flex items-center justify-center" style={{ zIndex: tokens.zIndex.modal }}>
          <div className="bg-white rounded-lg p-6 max-w-lg mx-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-4 break-words">Chart Settings</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2 break-words">Chart Type</label>
              ...
              <div className="flex space-x-2">
                <button className="flex-1 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">Apply</button>
                <button className="flex-1 px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 text-sm">Cancel</button>
              </div>
            </div>
          </div>
        </div>
