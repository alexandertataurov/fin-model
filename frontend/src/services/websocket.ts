...
export class WebSocketService {
  ...
  constructor(
    ...
    this.connectionManager = new EnhancedConnectionManager(globalWebSocketErrorHandler);
  }

  async connect(endpoint: string): Promise<void> {
    // If already connecting to the same endpoint, return existing promise
    if (this.isConnecting && this.lastEndpoint === endpoint && this.connectionPromise) {
    ...
    this.connectionPromise = new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onclose = (event) => {
          ...
          if (event.code !== 1000 && event.code !== 4001 && this.connectionManager.shouldReconnect()) {
    ...
    if (this.connectionManager.shouldReconnect()) {
      ...
      setTimeout(() => {
        if (this.lastEndpoint) {
          this.connect(this.lastEndpoint).catch((error) => {
  ...
  addConnectionListener(listener: (state: ConnectionState) => void): () => void {
    return this.connectionManager.getErrorHandler().addConnectionListener(listener);
  }
...
export const dashboardWebSocketService = new WebSocketService(wsBaseUrl, getAuthToken);

// Notifications WebSocket service
export const notificationsWebSocketService = new WebSocketService(wsBaseUrl, getAuthToken);

// Financial data WebSocket service
export const financialDataWebSocketService = new WebSocketService(wsBaseUrl, getAuthToken);

// Parameters WebSocket service
export const parametersWebSocketService = new WebSocketService(wsBaseUrl, getAuthToken);
...
export class WebSocketConnectionManager {
  ...
  getConnectionState(key: string): 'connecting' | 'connected' | 'disconnected' {
    return this.connectionStates.get(key) || 'disconnected';
  }

  getAllConnectionStates(): Record<string, 'connecting' | 'connected' | 'disconnected'> {
