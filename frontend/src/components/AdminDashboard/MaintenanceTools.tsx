...
export const MaintenanceTools: React.FC<MaintenanceToolsProps> = memo(
  ({ onOperationComplete }) => {
    ...
    const [runningOperations, setRunningOperations] = useState<Set<string>>(
      new Set()
    );
    const [selectedOperation, setSelectedOperation] =
      useState<MaintenanceOperation | null>(null);
    const [dryRunResult, setDryRunResult] = useState<MaintenanceResult | null>(
      null
    );
    ...
    const executeOperation = useCallback(
      async (operation: MaintenanceOperation, dryRun: boolean = false) => {
      ...
      [saveHistory, onOperationComplete]
    );
              ...
              <Card key={operation.id} className="relative">
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <Button
                      variant={
                        operation.risk === 'high' ? 'destructive' : 'default'
                      }
                      size="sm"
                      onClick={() => {
                  ...
                  {history.slice(0, 10).map(entry => (
                    <TableRow key={entry.id}>
                      <TableCell className="font-medium">
                        {MAINTENANCE_OPERATIONS.find(
                          op => op.type === entry.operation
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-muted rounded-lg">
                <div>
                  ...
                  <Badge
                    className={`ml-2 ${getRiskColor(selectedOperation?.risk || 'low')}`}
                  >
                ...
                >
                  ...
                  <AlertDescription>
                    <div className="space-y-2">
                      <p>
                        <strong>Dry Run Result:</strong> {dryRunResult.message}
                      </p>
                      <p>
                        <strong>Affected Items:</strong>{' '}
                        {dryRunResult.affectedItems}
                      </p>
              ...
              {selectedOperation?.risk === 'high' && (
                <Alert variant="destructive">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    <strong>High Risk Operation:</strong> This operation may
                    affect system performance or data integrity. Ensure you have
                    recent backups and proceed with caution.
                  </AlertDescription>
                </Alert>
              )}
            </div>
            ...
            <DialogFooter className="flex justify-between">
              <div className="flex space-x-2">
                {selectedOperation?.supportsDryRun && (
                  <Button
                    ...
                    onClick={() =>
                      selectedOperation &&
                      executeOperation(selectedOperation, true)
                    }
                    disabled={runningOperations.has(
              ...
              <div className="flex space-x-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    selectedOperation &&
                    executeOperation(selectedOperation, false)
                  }
