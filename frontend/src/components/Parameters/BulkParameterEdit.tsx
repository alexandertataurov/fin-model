...
export function BulkParameterEdit({
  ...
  const [applyMode, setApplyMode] = useState<'replace' | 'multiply' | 'add'>(
    'replace'
  );
  ...
  const handleParameterToggle = useCallback((id: string, selected: boolean) => {
    setUpdates(prev =>
      prev.map(update => (update.id === id ? { ...update, selected } : update))
    );
  }, []);
  ...
  const handleSave = useCallback(() => {
    const changedUpdates = updates.filter(
      u => u.selected && u.newValue !== u.currentValue
    );
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[80vh]">
        <DialogHeader>
          ...
          <DialogDescription>
            Select parameters and apply changes in bulk. Changes will trigger
            model recalculation.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button variant="outline" size="sm" onClick={handleSelectAll}>
                {filteredUpdates.every(u => u.selected)
                  ? 'Deselect All'
                  : 'Select All'}
              </Button>
                ...
                {filteredUpdates.map(update => {
                  ...
                  return (
                    <TableRow
                      key={update.id}
                      className={
                        hasChange && update.selected ? 'bg-muted/50' : ''
                      }
                    >
                      <TableCell>
                        <Checkbox
                      ...
                      <TableCell>
                        <Input
                          ...
                          onChange={e =>
                            handleValueChange(update.id, e.target.value)
                          }
          ...
          {hasChanges && (
            <Alert>
              ...
              <AlertDescription>
                ...
                parameters will be updated. This will trigger model
                recalculation.
              </AlertDescription>
            </Alert>
          )}
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={!hasChanges}>
            ...
            Apply Changes (
            {
              updates.filter(u => u.selected && u.newValue !== u.currentValue)
                .length
            }
            )
          </Button>
