...
interface ParameterPanelProps {
  ...
  onBulkParameterChange?: (updates: Array<{id: string, value: number}>) => void
  readOnly?: boolean
}
...
export function ParameterPanel({ 
  ...
  const [showImpactAnalysis, setShowImpactAnalysis] = useState<Parameter | null>(null)
  const [showHistory, setShowHistory] = useState<Parameter | null>(null)
  const [showTemplates, setShowTemplates] = useState(false)
  const [showExport, setShowExport] = useState(false)
  const [undoHistory, setUndoHistory] = useState<Array<{parameterId: string, oldValue: number, newValue: number}>>([])
  const [redoHistory, setRedoHistory] = useState<Array<{parameterId: string, oldValue: number, newValue: number}>>([])

  // Load parameter groups and data
  const loadParameterData = useCallback(async () => {
    try {
      const response = await fetch(`/api/v1/parameters/models/${modelId}/parameter-groups`)
      if (response.ok) {
  ...
  const handleParameterChange = useCallback(async (parameterId: string, value: number) => {
    ...
    try {
      const response = await fetch(`/api/v1/parameters/${parameterId}/validate`, {
        method: 'POST',
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Add to undo history
        const currentParam = [...parameterGroups.flatMap(g => g.parameters), ...ungroupedParameters]
  ...
  const handleBulkUpdate = useCallback(async (updates: Array<{id: string, value: number}>) => {
    ...
    try {
      const response = await fetch(`/api/v1/parameters/models/${modelId}/parameters/batch`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const resetParametersToDefault = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/v1/parameters/models/${modelId}/reset-parameters`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  }, [readOnly, parameterGroups, ungroupedParameters, modelId, loadParameterData])
  ...
  const filterParameters = useCallback((parameters: Parameter[]) => {
  ...
  const categories = [
    'all',
    ...new Set([
      ...parameterGroups.flatMap(group => group.parameters.map(p => p.category)),
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {parameterGroups.length > 0 && (
          <Accordion type="multiple" defaultValue={parameterGroups.map(g => g.id)}>
      ...
      {showHistory && (
        <ParameterHistory
          ...
          onRevertToValue={(value) => handleParameterChange(showHistory.id, value)}
        />
      )}
      ...
      {showExport && (
        <ParameterExport
          modelId={modelId}
          parameters={[...parameterGroups.flatMap(g => g.parameters), ...ungroupedParameters]}
