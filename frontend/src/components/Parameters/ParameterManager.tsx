...
const ParameterManager: React.FC<ParameterManagerProps> = ({
  ...
  const [parameters, setParameters] =
    useState<ParameterGroup[]>(initialParameters);
        ...
        {parameter.min_value !== undefined &&
          parameter.max_value !== undefined && (
            <div className="flex justify-between text-xs text-gray-500">
              <span>
                {parameter.is_percentage
                  ? `${(parameter.min_value * 100).toFixed(0)}%`
                  : parameter.min_value}
              </span>
              <span>
                {parameter.is_percentage
                  ? `${(parameter.max_value * 100).toFixed(0)}%`
                  : parameter.max_value}
              </span>
            </div>
          )}
        ...
        {hasUnsavedChanges && (
          <div className="mt-6 p-4 bg-orange-50 border border-orange-200 rounded-lg">
            ...
            <p className="text-sm text-orange-700 mt-1">
              Click "Save Changes" to apply your parameter updates to the
              financial model.
            </p>
          </div>
        )}
