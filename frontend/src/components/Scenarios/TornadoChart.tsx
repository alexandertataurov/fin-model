...
export const TornadoChart: React.FC<TornadoChartProps> = ({
  ...
  const maxImpact = Math.max(
    ...data.map(d => Math.max(Math.abs(d.low_impact - baseValue), Math.abs(d.high_impact - baseValue)))
  );
        ...
        <div className="space-y-4">
                ...
                <div key={index} className="relative">
                  ...
                  <div className="flex justify-between items-center mb-1">
                    <div className="flex items-center space-x-2">
                      <span className="text-sm font-medium truncate max-w-[200px]" title={item.parameter}>
                        {item.parameter}
                      </span>
                    ...
                    <div className="text-xs text-muted-foreground">
                      Impact: {Math.abs(highDiff - lowDiff) > 1000 ? formatValue(Math.abs(highDiff - lowDiff)) : Math.abs(highDiff - lowDiff).toFixed(1)}
                    </div>
                  </div>
                  ...
                  <div className="relative h-8 bg-gray-100 rounded-md">
                    ...
                    {lowDiff < 0 && (
                      <div
                        className={`absolute top-1 bottom-1 ${getBarColorClass(item.rank)} opacity-70 rounded-l-sm`}
                        style={{
                          right: '50%',
                          width: `${lowPercent}%`
                        }}
                    ...
                    {highDiff > 0 && (
                      <div
                        className={`absolute top-1 bottom-1 ${getBarColorClass(item.rank)} opacity-70 rounded-r-sm`}
                        style={{
                          left: '50%',
                          width: `${highPercent}%`
                        }}
          ...
          <div className="flex justify-center space-x-6 pt-4 border-t">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-muted-foreground">High Impact (Top 3)</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-orange-500 rounded"></div>
              <span className="text-xs text-muted-foreground">Medium Impact (4-6)</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-yellow-500 rounded"></div>
              <span className="text-xs text-muted-foreground">Low Impact (7-9)</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-blue-500 rounded"></div>
              <span className="text-xs text-muted-foreground">Minimal Impact (10+)</span>
            </div>
          </div>
          ...
          <div className="text-center pt-2">
            <p className="text-xs text-muted-foreground">
              Base value: {formatValue(baseValue)} | 
              Parameters ranked by sensitivity impact
            </p>
          </div>
        </div>
      </CardContent>
