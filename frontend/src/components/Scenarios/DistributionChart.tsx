...
export const DistributionChart: React.FC<DistributionChartProps> = ({
  ...
  // Calculate bin width
  const binWidth =
    data.bin_edges.length > 1 ? data.bin_edges[1] - data.bin_edges[0] : 1;
    ...
    <Card className={className}>
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>{title}</CardTitle>
          <Badge variant="secondary">{data.values.length} bins</Badge>
        </div>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <div className="flex justify-between mt-2 text-xs text-muted-foreground">
              ...
              <span>
                {formatValue(
                  data.bin_edges[Math.floor(data.bin_edges.length / 2)] || 0
                )}
              </span>
              <span>
                {formatValue(data.bin_edges[data.bin_edges.length - 1] || 0)}
              </span>
            </div>
          </div>
                  ...
                  {stats.percentile_50 !== undefined && (
                    <div className="text-center p-2 bg-blue-50 rounded">
                      <div className="font-semibold text-blue-700">
                        {formatValue(stats.percentile_50)}
                      </div>
          ...
          {showStats &&
            stats &&
            stats.percentile_25 !== undefined &&
            stats.percentile_75 !== undefined && (
              <div className="space-y-2">
