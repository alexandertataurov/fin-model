              ...
              <>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div className="text-center">
                          <div className="flex items-center justify-center mb-2">
                            <div className={`w-3 h-3 rounded-full mr-2 ${
                              systemMetrics?.cpu_usage && systemMetrics.cpu_usage > 80 ? 'bg-red-500' :
                              systemMetrics?.cpu_usage && systemMetrics.cpu_usage > 60 ? 'bg-yellow-500' : 'bg-green-500'
                            }`}></div>
                        ...
                        <div className="text-center">
                          <div className="flex items-center justify-center mb-2">
                            <div className={`w-3 h-3 rounded-full mr-2 ${
                              systemMetrics?.memory_usage && systemMetrics.memory_usage > 80 ? 'bg-red-500' :
                              systemMetrics?.memory_usage && systemMetrics.memory_usage > 60 ? 'bg-yellow-500' : 'bg-green-500'
                            }`}></div>
                        ...
                        <div className="text-center">
                          <div className="flex items-center justify-center mb-2">
                            <div className={`w-3 h-3 rounded-full mr-2 ${
                              systemMetrics?.disk_usage && systemMetrics.disk_usage > 80 ? 'bg-red-500' :
                              systemMetrics?.disk_usage && systemMetrics.disk_usage > 60 ? 'bg-yellow-500' : 'bg-green-500'
                            }`}></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <Card className="hover:shadow-md transition-shadow">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-blue-600">{formatNumber(systemStats.users.total)}</div>
                      ...
                      <div className="flex items-center mt-2">
                        <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                          <div 
                            ...
                            style={{ width: `${(systemStats.users.active / systemStats.users.total) * 100}%` }}
                          ></div>
                        </div>
                        <span className="text-xs text-muted-foreground ml-2">
                          {Math.round((systemStats.users.active / systemStats.users.total) * 100)}% active
                        </span>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="hover:shadow-md transition-shadow">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-green-600">{formatNumber(systemStats.files.total)}</div>
                      <div className="text-xs text-muted-foreground mt-1">
                        {systemStats.files.completed} completed • {systemStats.files.processing} processing
                      </div>
                      <div className="flex items-center mt-2">
                        <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                          <div 
                            className="bg-green-500 h-1.5 rounded-full" 
                            style={{ width: `${(systemStats.files.completed / systemStats.files.total) * 100}%` }}
                          ></div>
                        </div>
                        <span className="text-xs text-muted-foreground ml-2">
                          {Math.round((systemStats.files.completed / systemStats.files.total) * 100)}% done
                        </span>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="hover:shadow-md transition-shadow">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-purple-600">{formatNumber(systemStats.financial_data.statements)}</div>
                      <div className="text-xs text-muted-foreground mt-1">
                        statements • {formatNumber(systemStats.financial_data.parameters)} parameters
                      </div>
                  ...
                  <Card className="hover:shadow-md transition-shadow">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-orange-600">{systemStats.system.database_size}</div>
                      <div className="text-xs text-muted-foreground mt-1">
                        ...
                        <div className="space-y-6">
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`text-sm font-medium ${
                                systemMetrics.cpu_usage && systemMetrics.cpu_usage > 80 ? 'text-red-500' :
                                systemMetrics.cpu_usage && systemMetrics.cpu_usage > 60 ? 'text-yellow-500' : 'text-green-500'
                              }`}>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`text-sm font-medium ${
                                systemMetrics.memory_usage && systemMetrics.memory_usage > 80 ? 'text-red-500' :
                                systemMetrics.memory_usage && systemMetrics.memory_usage > 60 ? 'text-yellow-500' : 'text-green-500'
                              }`}>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`text-sm font-medium ${
                                systemMetrics.disk_usage && systemMetrics.disk_usage > 80 ? 'text-red-500' :
                                systemMetrics.disk_usage && systemMetrics.disk_usage > 60 ? 'text-yellow-500' : 'text-green-500'
                              }`}>
                        ...
                        <div className="mt-6 pt-4 border-t">
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <span className="font-medium">{formatNumber(systemMetrics.request_count_24h || 0)}</span>
                            </div>
                          </div>
                        </div>
                      </CardContent>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="space-y-4">
                          ...
                          <>
                            {(systemMetrics.cpu_usage && systemMetrics.cpu_usage > 90) && (
                              <Alert>
                                ...
                                <AlertDescription className="text-sm">
                                  <div className="font-medium text-red-600">High CPU Usage</div>
                                  <div className="text-muted-foreground">
                                    CPU usage is at {formatPercentage(systemMetrics.cpu_usage)}. Consider investigating running processes.
                                  </div>
                                </AlertDescription>
                              </Alert>
                            )}
                            ...
                            {(systemMetrics.memory_usage && systemMetrics.memory_usage > 90) && (
                              <Alert>
                                <AlertCircle className="h-4 w-4" />
                                <AlertDescription className="text-sm">
                                  <div className="font-medium text-red-600">High Memory Usage</div>
                                  <div className="text-muted-foreground">
                                    Memory usage is at {formatPercentage(systemMetrics.memory_usage)}. System may become unstable.
                                  </div>
                                </AlertDescription>
                              </Alert>
                            )}
                        ...
                        {/* Security Alerts */}
                        {securityAudit && securityAudit.failed_logins_24h > 10 && (
                          <Alert>
                            <Shield className="h-4 w-4" />
                            <AlertDescription className="text-sm">
                              <div className="font-medium text-yellow-600">Multiple Failed Logins</div>
                              <div className="text-muted-foreground">
                                {securityAudit.failed_logins_24h} failed login attempts in the last 24 hours.
                              </div>
                            </AlertDescription>
                          </Alert>
                        )}
                        ...
                        {(!systemMetrics || !systemStats) && (
                          <Alert>
                            <AlertCircle className="h-4 w-4" />
                            <AlertDescription className="text-sm">
                              <div className="font-medium">System Monitoring Limited</div>
                              <div className="text-muted-foreground">
                                Some monitoring data is unavailable. Check system health.
                              </div>
                            </AlertDescription>
                          </Alert>
                        )}
                    ...
                    <div className="space-y-3">
                      {userActivity.length > 0 ? (
                        userActivity.slice(0, 6).map((activity) => (
                          <div key={activity.user_id} className="flex items-center justify-between p-3 rounded-lg border hover:bg-accent/50 transition-colors">
                      ...
                      <div className="grid grid-cols-3 gap-4 text-center">
                        ...
                        <div>
                          <div className="text-lg font-bold text-green-600">
                            {userActivity.reduce((sum, u) => sum + u.files_uploaded, 0)}
                          </div>
                          <div className="text-xs text-muted-foreground">Files Uploaded</div>
                        </div>
                        <div>
                          <div className="text-lg font-bold text-purple-600">
                            {userActivity.reduce((sum, u) => sum + u.models_created, 0)}
                          </div>
