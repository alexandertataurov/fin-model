...
describe('AdminDashboard Overview and Maintenance', () => {
  beforeEach(() => {
    ...
    const mockFunctions = {
      ...
      clearRateLimits: vi
        .fn()
        .mockResolvedValue({ message: 'Cleared', cleared_records: 1 }),
    };
  ...
  it('shows High CPU Usage alert when cpu_usage > 90', async () => {
    mocked.getSystemMetrics = vi.fn().mockResolvedValue(baseMetricsHighCpu);
  ...
  it('runs maintenance: preview cleanup and run cleanup', async () => {
    ...
    const previewBtn = await screen.findByRole('button', {
      name: /preview cleanup/i,
    });
    await userEvent.click(previewBtn);
    expect(mocked.cleanupFiles).toHaveBeenCalledWith(true);
