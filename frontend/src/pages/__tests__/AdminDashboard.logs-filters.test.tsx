...
describe('AdminDashboard Logs filters', () => {
  ...
  it('applies level/limit/date/search and calls service with correct params', async () => {
    ...
    const dateInputs = Array.from(
      logsSection.querySelectorAll('input[type="date"]')
    ) as HTMLInputElement[];
    ...
    const searchBox = logsSection.querySelector(
      'input[type="text"][placeholder="Search"]'
    ) as HTMLInputElement;
    await userEvent.type(searchBox, 'timeout');

    // Refresh Logs button
    const refreshBtn = Array.from(logsSection.querySelectorAll('button')).find(
      b => /refresh logs/i.test(b.textContent || '')
    ) as HTMLButtonElement;
    await userEvent.click(refreshBtn);

    expect(mocked.getSystemLogs).toHaveBeenCalledWith(
      'INFO',
      50,
