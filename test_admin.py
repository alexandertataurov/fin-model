...
def test_admin_endpoints():
    ...
    for endpoint in endpoints:
        try:
            ...
            print(
                f"{status_icon} {endpoint} - Expected: {expected_status}, Got: {actual_status}"
            )
    ...
    if successful == total:
        print(
            "SUCCESS: All admin endpoints are properly implemented and secured!"
        )
...
def test_backend_structure():
    ...
    try:
        ...
        response = requests.get(
            f"{BASE_URL.replace('/api/v1', '')}/docs", timeout=5
        )
    ...
    try:
        response = requests.get(
            f"{BASE_URL.replace('/admin', '')}/health", timeout=5
        )
...
if __name__ == "__main__":
    ...
    print(
        "Note: 401 Unauthorized responses are expected for unauthenticated requests"
    )
