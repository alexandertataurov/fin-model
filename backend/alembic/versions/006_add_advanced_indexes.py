...
def upgrade():
    ...
    op.create_index('ix_users_created_date', 'users', [sa.text('DATE(created_at)')], unique=False)
    ...
    op.create_index('ix_files_status_created', 'uploaded_files', ['status', 'created_at'], unique=False)
    op.create_index('ix_files_size_type', 'uploaded_files', ['file_size', 'file_type'], unique=False)
    op.create_index('ix_files_processing_times', 'uploaded_files', ['processing_started_at', 'processing_completed_at'], unique=False)
    op.create_index('ix_files_user_status', 'uploaded_files', ['uploaded_by_id', 'status'], unique=False)
    
    # Advanced indexes for parameters table
    op.create_index('ix_parameters_type_category', 'parameters', ['parameter_type', 'category'], unique=False)
    op.create_index('ix_parameters_sensitivity_editable', 'parameters', ['sensitivity_level', 'is_editable'], unique=False)
    op.create_index('ix_parameters_file_sheet', 'parameters', ['source_file_id', 'source_sheet'], unique=False)
    op.create_index('ix_parameters_updated_recently', 'parameters', [sa.text('updated_at DESC')], unique=False)
    
    # Advanced indexes for scenarios table
    op.create_index('ix_scenarios_status_baseline', 'scenarios', ['status', 'is_baseline'], unique=False)
    op.create_index('ix_scenarios_file_created', 'scenarios', ['base_file_id', 'created_at'], unique=False)
    op.create_index('ix_scenarios_calculation_status', 'scenarios', ['calculation_status', 'last_calculated_at'], unique=False)
    op.create_index('ix_scenarios_user_active', 'scenarios', ['created_by_id', 'status'], unique=False)
    
    # Advanced indexes for financial_statements table
    op.create_index('ix_statements_period_range', 'financial_statements', ['period_start', 'period_end', 'statement_type'], unique=False)
    op.create_index('ix_statements_scenario_type', 'financial_statements', ['scenario_id', 'statement_type', 'period_start'], unique=False)
    op.create_index('ix_statements_currency_period', 'financial_statements', ['currency', 'period_type'], unique=False)
    op.create_index('ix_statements_baseline_version', 'financial_statements', ['is_baseline', 'version'], unique=False)
    
    # Advanced indexes for metrics table
    op.create_index('ix_metrics_name_period', 'metrics', ['metric_name', 'period_start', 'period_end'], unique=False)
    op.create_index('ix_metrics_category_type', 'metrics', ['metric_category', 'metric_type'], unique=False)
    op.create_index('ix_metrics_scenario_category', 'metrics', ['scenario_id', 'metric_category'], unique=False)
    ...
    op.create_index('ix_timeseries_type_date', 'time_series', ['data_type', 'period_date'], unique=False)
    op.create_index('ix_timeseries_scenario_type_date', 'time_series', ['scenario_id', 'data_type', 'period_date'], unique=False)
    op.create_index('ix_timeseries_frequency_actual', 'time_series', ['frequency', 'is_actual'], unique=False)
    op.create_index('ix_timeseries_date_range', 'time_series', [sa.text('period_date DESC')], unique=False)
    op.create_index('ix_timeseries_subtype_date', 'time_series', ['data_subtype', 'period_date'], unique=False)
    
    # Advanced indexes for calculations table
    op.create_index('ix_calculations_order_active', 'calculations', ['execution_order', 'is_active'], unique=False)
    op.create_index('ix_calculations_type_scenario', 'calculations', ['calculation_type', 'scenario_id'], unique=False)
    op.create_index('ix_calculations_executed_recently', 'calculations', [sa.text('last_executed_at DESC NULLS LAST')], unique=False)
    
    # Advanced indexes for templates table
    op.create_index('ix_templates_type_active', 'templates', ['template_type', 'is_active'], unique=False)
    op.create_index('ix_templates_system_usage', 'templates', ['is_system_template', 'usage_count'], unique=False)
    
    # Advanced indexes for file_versions table
    op.create_index('ix_versions_file_current', 'file_versions', ['file_id', 'is_current'], unique=False)
    op.create_index('ix_versions_hash_lookup', 'file_versions', ['file_hash'], unique=False)
    op.create_index('ix_versions_change_type', 'file_versions', ['change_type', 'created_at'], unique=False)
    ...
    op.create_index('ix_audit_user_success', 'audit_logs', ['user_id', 'success', 'created_at'], unique=False)
    op.create_index('ix_audit_resource_type', 'audit_logs', ['resource', 'resource_id'], unique=False)
    op.create_index('ix_audit_recent', 'audit_logs', [sa.text('created_at DESC')], unique=False)
    op.create_index('ix_audit_ip_address', 'audit_logs', ['ip_address'], unique=False)
    
    # Advanced indexes for reports
    op.create_index('ix_report_templates_active', 'report_templates', ['is_active', 'report_type'], unique=False)
    op.create_index('ix_report_schedules_next_run', 'report_schedules', ['next_run_at', 'is_active'], unique=False)
    op.create_index('ix_report_exports_status_created', 'report_exports', ['status', 'created_at'], unique=False)
    op.create_index('ix_report_exports_user_format', 'report_exports', ['created_by', 'export_format'], unique=False)
    ...
    op.execute("CREATE INDEX CONCURRENTLY ix_users_active_only ON users (id) WHERE is_active = true")
    op.execute("CREATE INDEX CONCURRENTLY ix_files_processing ON uploaded_files (id, created_at) WHERE status IN ('processing', 'uploaded')")
    op.execute("CREATE INDEX CONCURRENTLY ix_scenarios_active ON scenarios (id, updated_at) WHERE status = 'active'")
    op.execute("CREATE INDEX CONCURRENTLY ix_calculations_pending ON calculations (execution_order) WHERE is_active = true AND last_executed_at IS NULL")
