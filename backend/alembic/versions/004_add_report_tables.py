...
def upgrade():
    ...
    op.create_table('report_templates',
        ...
        sa.Column('report_type', sa.Enum('FINANCIAL_SUMMARY', 'PROFIT_LOSS', 'BALANCE_SHEET', 'CASH_FLOW', 'CUSTOM', name='reporttype'), nullable=False),
    ...
    op.create_table('report_schedules',
        ...
        sa.Column('export_format', sa.Enum('PDF', 'EXCEL', 'CSV', 'PNG', 'SVG', 'JSON', name='exportformat'), nullable=True),
    ...
    op.create_table('report_exports',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('name', sa.String(length=255), nullable=False),
        sa.Column('export_format', sa.Enum('PDF', 'EXCEL', 'CSV', 'PNG', 'SVG', 'JSON', name='exportformat'), nullable=False),
        sa.Column('status', sa.Enum('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'CANCELLED', name='reportstatus'), nullable=True),
    ...
    op.create_index('ix_report_templates_type', 'report_templates', ['report_type'], unique=False)
    op.create_index('ix_report_templates_created_by', 'report_templates', ['created_by'], unique=False)
    op.create_index('ix_report_schedules_template_id', 'report_schedules', ['template_id'], unique=False)
    op.create_index('ix_report_schedules_created_by', 'report_schedules', ['created_by'], unique=False)
    op.create_index('ix_report_exports_status', 'report_exports', ['status'], unique=False)
    op.create_index('ix_report_exports_created_by', 'report_exports', ['created_by'], unique=False)
    op.create_index('ix_report_exports_template_id', 'report_exports', ['template_id'], unique=False)
    op.create_index('ix_report_exports_expires_at', 'report_exports', ['expires_at'], unique=False)
