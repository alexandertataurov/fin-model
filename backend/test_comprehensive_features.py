...
class FinVisionTester:
    ...
    def test_file_upload(self, file_path: str):
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                files = {"file": ("test_financial_model.xlsx", f, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")}
            ...
            if response.status_code == 200:
                ...
                print(f"   Virus scan: {'Clean' if result['virus_scan_clean'] else 'Threats detected'}")
                return True
            else:
        ...
        while time.time() - start_time < max_wait:
            try:
                response = requests.get(
                    f"{API_BASE}/files/task/{task_id}/status",
                    headers=headers
                )
    ...
    def test_analytics_dashboard(self):
        ...
        try:
            ...
            response = requests.get(
                f"{API_BASE}/analytics/dashboard?days=7",
                headers=headers
            )
            ...
            if response.status_code == 200:
                ...
                print(f"   Avg processing time: {overview.get('average_processing_time_minutes', 0)} min")
    ...
    def test_partial_processing(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   Can partial process: {result.get('can_partial_process', False)}")
                print(f"   Estimated completion: {result.get('estimated_completion', 0)}%")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for i, option in enumerate(options[:3]):  # Show first 3 options
                    ...
                    print(f"     Success rate: {option['estimated_success_rate']*100:.1f}%")
    ...
    def test_manual_intervention(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                response = requests.get(
                    f"{API_BASE}/admin-tools/interventions/pending",
                    headers=headers
                )
                
                if response.status_code == 200:
                    pending = response.json()
                    print(f"   Pending interventions: {pending.get('total_pending', 0)}")
    ...
    def test_system_health(self):
        ...
        try:
            ...
            # Get system health
            response = requests.get(
                f"{API_BASE}/admin-tools/system/health",
                headers=headers
            )
...
def main():
    ...
    print("This script tests all major functionality of the file upload and processing system.")
