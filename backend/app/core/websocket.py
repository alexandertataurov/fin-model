...
class ConnectionManager:
    ...
    async def broadcast_file_status(self, file_id: int, status_data: Dict[str, Any], user_id: int):
        """Broadcast file status update to subscribed users."""
        # Only send to the user who owns the file
        if user_id in self.file_subscriptions and file_id in self.file_subscriptions[user_id]:
            await self.send_to_user(user_id, {
    ...
    async def broadcast_task_progress(self, task_id: str, progress_data: Dict[str, Any], user_id: int):
        """Broadcast task progress update to subscribed users."""
        if user_id in self.task_subscriptions and task_id in self.task_subscriptions[user_id]:
            await self.send_to_user(user_id, {
