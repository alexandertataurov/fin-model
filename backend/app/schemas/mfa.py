...
class AuthenticationFlowResponse(BaseModel):
    ...
    status: str = Field(..., description="Status: success, mfa_required, webauthn_required")
    access_token: Optional[str] = Field(None, description="JWT access token if authentication complete")
    ...
    available_methods: Optional[List[str]] = Field(None, description="Available 2FA methods")
