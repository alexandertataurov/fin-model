...
class FileRetentionPolicy:
    ...
    def applies_to_file(self, file_record: UploadedFile, user: User = None) -> bool:
        ...
        # Check file type
        if self.applies_to_file_types and file_record.file_type not in self.applies_to_file_types:
            return False
...
class FileCleanupService:
        ...
        with SessionLocal() as db:
            try:
                for policy in self.policies:
                    ...
                    policy_results = await self._apply_cleanup_policy(db, policy, dry_run)
                    cleanup_results["policies_applied"][policy.name] = policy_results
                    cleanup_results["files_processed"] += policy_results["files_identified"]
                    cleanup_results["files_deleted"] += policy_results["files_deleted"]
                    cleanup_results["storage_freed_mb"] += policy_results["storage_freed_mb"]
                    cleanup_results["errors"].extend(policy_results["errors"])
                
                logger.info(f"Cleanup completed: {cleanup_results['files_deleted']} files deleted, "
    ...
    async def _apply_cleanup_policy(self, db: Session, policy: FileRetentionPolicy, dry_run: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            if policy.applies_to_file_types:
                query = query.filter(UploadedFile.file_type.in_(policy.applies_to_file_types))
            ...
            for file_record in files_to_cleanup:
                try:
                    ...
                    user = db.query(User).filter(User.id == file_record.uploaded_by_id).first()
                    ...
                    if not dry_run:
                        ...
                        if file_deleted:
                            ...
                            policy_results["storage_freed_mb"] += file_record.file_size / (1024 * 1024)
                            
                            logger.debug(f"Deleted file {file_record.original_filename} (Policy: {policy.name})")
                    else:
                        # Dry run - just count
                        policy_results["files_deleted"] += 1
                        policy_results["storage_freed_mb"] += file_record.file_size / (1024 * 1024)
    ...
    async def cleanup_orphaned_files(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if upload_folder.exists():
                results.update(await self._cleanup_local_orphans(upload_folder, dry_run))
    ...
    async def _cleanup_local_orphans(self, upload_folder: Path, dry_run: bool) -> Dict[str, Any]:
    ...
    async def cleanup_processing_logs(self, retention_days: int = 30, dry_run: bool = False) -> Dict[str, Any]:
        """
    ...
    async def run_scheduled_cleanup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Cleanup old logs
            log_cleanup = await self.cleanup_processing_logs(retention_days=30, dry_run=False)
