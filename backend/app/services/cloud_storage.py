...
class CloudStorageInterface(ABC):
    ...
    @abstractmethod
    async def upload_file(self, file_data: BinaryIO, file_path: str, metadata: Optional[Dict[str, str]] = None) -> str:
...
class AzureBlobStorageService(CloudStorageInterface):
    ...
    def __init__(self):
        ...
        self.container_name = getattr(settings, 'AZURE_CONTAINER_NAME', 'finvision-files')
        connection_string = getattr(settings, 'AZURE_STORAGE_CONNECTION_STRING', None)
        
        if not connection_string:
            raise ValueError("Azure Storage connection string not found. Please configure AZURE_STORAGE_CONNECTION_STRING")
...
class CloudStorageManager:
    ...
    def _initialize_storage_provider(self) -> CloudStorageInterface:
        ...
        if provider_type == 's3' and AWS_AVAILABLE:
            try:
                ...
                print(f"Failed to initialize S3 storage: {e}. Falling back to local storage.")
            ...
            try:
                ...
                print(f"Failed to initialize Azure storage: {e}. Falling back to local storage.")
                return LocalStorageService()
        
        else:
            return LocalStorageService()
    
    async def upload_file(self, file_data: BinaryIO, filename: str, user_id: int, metadata: Optional[Dict[str, str]] = None) -> str:
        ...
        return await self.storage_provider.upload_file(file_data, organized_path, upload_metadata)
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "features": {
                "encryption": True,
                "versioning": isinstance(self.storage_provider, (S3StorageService, AzureBlobStorageService)),
                "cdn": isinstance(self.storage_provider, (S3StorageService, AzureBlobStorageService)),
                "backup": isinstance(self.storage_provider, (S3StorageService, AzureBlobStorageService))
            }
        } 