...
class PDFReportGenerator:
    ...
    def __init__(self, output_dir: Optional[str] = None):
        self.output_dir = Path(output_dir) if output_dir else Path(settings.UPLOAD_DIR) / "reports"
        ...
        if isinstance(metrics_data, dict):
            for key, value in metrics_data.items():
                if isinstance(value, (int, float)):
                    formatted_value = self._format_currency(value) if 'revenue' in key.lower() or 'profit' in key.lower() else f"{value:,.2f}"
    ...
    def _build_charts_section(self, charts_data: Dict[str, Any]) -> List:
        ...
        for chart_name, chart_data in charts_data.items():
            ...
            story.append(Paragraph(chart_name.replace('_', ' ').title(), self.styles['MetricTitle']))
    ...
    def _generate_chart(self, chart_type: str, data: List[Dict[str, Any]]) -> Optional[Image]:
    ...
    def _create_bar_chart(self, ax, df: pd.DataFrame, chart_type: str):
        """Create bar chart."""
        if 'period' in df.columns and 'value' in df.columns:
            bars = ax.bar(df['period'], df['value'], color=self.chart_config['colors'][:len(df)])
    ...
    def _build_tables_section(self, tables_data: Dict[str, Any]) -> List:
        ...
        for table_name, table_data in tables_data.items():
            ...
            story.append(Paragraph(table_name.replace('_', ' ').title(), self.styles['MetricTitle']))
