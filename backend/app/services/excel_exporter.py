...
class ExcelExporter:
    ...
    def _create_sheet(
        ...
        if "time_series" in data:
            current_row = self._add_time_series_section(
                ws, data["time_series"], current_row
            )
    ...
    def _add_metrics_section(
        self, worksheet, metrics_data: Dict[str, Any], start_row: int
    ) -> int:
        ...
        if isinstance(metrics_data, dict):
            ...
            for metric_name, metric_value in metrics_data.items():
                worksheet.cell(
                    row=current_row,
                    column=1,
                    value=metric_name.replace("_", " ").title(),
                )
                ...
                if isinstance(metric_value, dict):
                    ...
                    worksheet.cell(
                        row=current_row,
                        column=3,
                        value=metric_value.get("previous", ""),
                    )
                    ...
                    if metric_value.get("current") and metric_value.get(
                        ...
                        try:
                            ...
                            ).number_format = self.percentage_format
    ...
    def _add_time_series_section(
        ...
        for col_idx, column_name in enumerate(df.columns, 1):
            cell = worksheet.cell(
                row=current_row,
                column=col_idx,
                value=column_name.replace("_", " ").title(),
            )
            cell.font = self.header_font
            cell.fill = self.header_fill
            cell.border = self.thin_border
    ...
    def _create_line_chart(
        self, worksheet, data: List[Dict], start_row: int, start_col: int
    ):
