...
class EnhancedChartMethods:
    ...
    def __init__(self, chart_service: ChartDataService, metrics_service: MetricsCalculationService):
    ...
    async def generate_enhanced_dashboard_charts(
        ...
        try:
            ...
            chart_data["revenue_trend"] = self.chart_service.to_recharts_format(revenue_chart)
        ...
        try:
            ...
            if cf_statements:
                cf_chart = self.chart_service.create_cash_flow_trend_chart(cf_statements)
                chart_data["cash_flow_trend"] = self.chart_service.to_recharts_format(cf_chart)
        except Exception as e:
        ...
        try:
            ...
            if pl_statements:
                ...
                expense_chart = self.chart_service.create_expense_breakdown_chart(latest_pl)
                chart_data["expense_breakdown"] = self.chart_service.to_recharts_format(expense_chart)
        except Exception as e:
        ...
        try:
            ...
            if bs_statements:
                ...
                asset_chart = self.chart_service.create_asset_composition_chart(latest_bs)
                chart_data["asset_composition"] = self.chart_service.to_recharts_format(asset_chart)
        except Exception as e:
        ...
        try:
            ...
            if cf_statements:
                ...
                waterfall_chart = self.chart_service.create_cash_flow_waterfall_chart(latest_cf)
                chart_data["cash_flow_waterfall"] = self.chart_service.to_recharts_format(waterfall_chart)
        except Exception as e:
        ...
        try:
            ...
            if len(pl_statements) >= 2:  # Need at least 2 periods for trend
                margin_chart = self.chart_service.create_margin_analysis_chart(pl_statements)
                chart_data["margin_analysis"] = self.chart_service.to_recharts_format(margin_chart)
        except Exception as e:
        ...
        try:
            ...
            if len(bs_statements) >= 2:  # Need at least 2 periods for trend
                liquidity_chart = self.chart_service.create_liquidity_ratios_chart(bs_statements)
                chart_data["liquidity_ratios"] = self.chart_service.to_recharts_format(liquidity_chart)
        except Exception as e:
        ...
        try:
            if len(statements) >= 2:  # Need at least 2 statements for comparison
                comparison_chart = self.chart_service.create_period_comparison_chart(
                    statements, ["revenue", "net_income", "total_assets"]
                )
                chart_data["period_comparison"] = self.chart_service.to_recharts_format(comparison_chart)
        except Exception as e:
    ...
    async def generate_enhanced_pl_charts(self, statement: FinancialStatement, user_statements: List[FinancialStatement]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(user_statements) > 1:
                ...
                charts["revenue_trend"] = self.chart_service.to_recharts_format(revenue_chart)
            
            # Expense Breakdown
            expense_chart = self.chart_service.create_expense_breakdown_chart(statement)
            charts["expense_breakdown"] = self.chart_service.to_recharts_format(expense_chart)
            
            # Margin Analysis
            if len(user_statements) > 1:
                margin_chart = self.chart_service.create_margin_analysis_chart(user_statements)
                charts["margin_analysis"] = self.chart_service.to_recharts_format(margin_chart)
            ...
            if len(user_statements) >= 2:
                ...
                charts["period_comparison"] = self.chart_service.to_recharts_format(comparison_chart)
    ...
    async def generate_enhanced_balance_sheet_charts(self, statement: FinancialStatement, user_statements: List[FinancialStatement]) -> Dict[str, Any]:
        ...
        try:
            ...
            charts["asset_composition"] = self.chart_service.to_recharts_format(asset_chart)
            
            # Liquidity Ratios Trend
            if len(user_statements) > 1:
                liquidity_chart = self.chart_service.create_liquidity_ratios_chart(user_statements)
                charts["liquidity_ratios"] = self.chart_service.to_recharts_format(liquidity_chart)
            ...
            if len(user_statements) >= 2:
                ...
                charts["balance_sheet_comparison"] = self.chart_service.to_recharts_format(comparison_chart)
    ...
    async def generate_enhanced_cash_flow_charts(self, statement: FinancialStatement, user_statements: List[FinancialStatement]) -> Dict[str, Any]:
        ...
        try:
            # Cash Flow Waterfall
            waterfall_chart = self.chart_service.create_cash_flow_waterfall_chart(statement)
            charts["cash_flow_waterfall"] = self.chart_service.to_recharts_format(waterfall_chart)
            
            # Cash Flow Trends
            if len(user_statements) > 1:
                trend_chart = self.chart_service.create_cash_flow_trend_chart(user_statements)
                charts["cash_flow_trend"] = self.chart_service.to_recharts_format(trend_chart)
            ...
            if len(user_statements) >= 2:
                ...
                charts["cash_flow_comparison"] = self.chart_service.to_recharts_format(comparison_chart)
    ...
    def calculate_enhanced_key_metrics(self, statements: List[FinancialStatement]) -> Dict[str, Any]:
        """Calculate enhanced key metrics using MetricsCalculationService."""
        
        try:
            # Calculate comprehensive financial ratios
            financial_ratios = self.metrics_service.calculate_financial_ratios(statements)
            
            # Get latest statements by type
            latest_pl = next((s for s in statements if s.statement_type == StatementType.PROFIT_LOSS), None)
            latest_bs = next((s for s in statements if s.statement_type == StatementType.BALANCE_SHEET), None)
            latest_cf = next((s for s in statements if s.statement_type == StatementType.CASH_FLOW), None)
            ...
            if latest_pl:
                ...
                revenue = self._extract_metric_value(line_items, ["revenue", "total_revenue"])
                if revenue:
                    metrics["revenue"] = {
                        "value": revenue,
                        "unit": "USD",
            ...
            if latest_cf:
                ...
                operating_cf = self._extract_metric_value(line_items, ["operating_cash_flow"])
                if operating_cf:
    ...
    def get_time_series_chart_data(
        ...
        try:
            ...
            # Generate forecast
            forecast = self.metrics_service.generate_simple_forecast(time_series, periods_ahead=3)
    ...
    def get_chart_data_for_frontend(self, chart_dataset: Dict[str, Any]) -> Dict[str, Any]:
