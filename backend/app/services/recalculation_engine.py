...
class IncrementalCalculator:
    ...
    def recalculate_from_parameters(
        ...
        try:
            ...
            for cell_ref in calc_order:
                try:
                    new_value = self._calculate_cell_value(
    ...
    def calculate_impact_analysis(
        ...
        for i in range(steps + 1):
            ...
            calc_result = self.recalculate_from_parameters(
                model_id, file_path, changed_params
            )
    ...
    def get_calculation_dependencies(
        self, file_path: str, cell_refs: List[str]
    ) -> DependencyGraph:
        """Get dependency graph for specific cells."""
        try:
            # Use formula engine to parse dependencies
            dependencies = self.formula_engine.analyze_dependencies(
                file_path, cell_refs
            )
    ...
    def _find_affected_cells(
        self, graph: DependencyGraph, changed_cells: List[str]
    ) -> Set[str]:
    ...
    def _get_calculation_order(
        self, graph: DependencyGraph, affected_cells: Set[str]
    ) -> List[str]:
    ...
    def _calculate_sensitivity_summary(
        ...
        sensitive_cells = sorted(
            max_sensitivity.items(), key=lambda x: x[1], reverse=True
        )
...
class ParameterValidator:
    ...
    def validate_parameter_value(
        self, parameter: Dict[str, Any], value: float
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                def _eval(node):
                    ...
                    if isinstance(node, ast.BoolOp):
                        return bool_ops[type(node.op)](
                            [_eval(v) for v in node.values]
                        )
                    if isinstance(node, ast.Compare):
                        left = _eval(node.left)
                        for op_node, comp in zip(
