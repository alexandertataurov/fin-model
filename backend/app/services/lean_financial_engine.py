...
class LeanFinancialEngine:
    ...
    def calculate_profit_loss(self, parameters: CoreParameters, base_revenue: float = 1000000) -> ProfitLossStatement:
        """Calculate comprehensive P&L statement"""
        pl = ProfitLossStatement()
        
        # Revenue Calculations
        pl.product_revenue = base_revenue * 0.7 * (1 + parameters.product_revenue_growth)
        pl.service_revenue = base_revenue * 0.25 * (1 + parameters.service_revenue_growth)
        pl.licensing_revenue = base_revenue * 0.03
        pl.other_revenue = base_revenue * 0.02
        pl.total_revenue = pl.product_revenue + pl.service_revenue + pl.licensing_revenue + pl.other_revenue
        
        # COGS Calculations
        pl.direct_materials = pl.total_revenue * parameters.direct_materials_percentage * (1 + parameters.materials_inflation_rate)
        pl.direct_labor = pl.total_revenue * parameters.direct_labor_percentage * (1 + parameters.labor_cost_inflation)
        pl.manufacturing_overhead = pl.total_revenue * parameters.manufacturing_overhead_percentage
        pl.freight_and_delivery = pl.total_revenue * 0.02
        pl.total_cogs = pl.direct_materials + pl.direct_labor + pl.manufacturing_overhead + pl.freight_and_delivery
        
        # Gross Profit
        pl.gross_profit = pl.total_revenue - pl.total_cogs
        pl.gross_margin_percentage = pl.gross_profit / pl.total_revenue if pl.total_revenue > 0 else 0
        ...
        pl.executive_salaries = pl.total_revenue * parameters.executive_salaries_percentage
        ...
        pl.total_operating_expenses = (pl.sales_commission + pl.marketing_spend + pl.advertising + 
                                     pl.rnd_expenses + pl.executive_salaries + pl.admin_salaries + 
                                     pl.office_rent + pl.utilities + pl.insurance)
        
        # Operating Income
        pl.operating_income = pl.gross_profit - pl.total_operating_expenses
        pl.operating_margin_percentage = pl.operating_income / pl.total_revenue if pl.total_revenue > 0 else 0
        ...
        pl.total_other_income_expense = pl.interest_income - pl.interest_expense + pl.foreign_exchange_gain_loss
        
        # Income Before Tax
        pl.income_before_tax = pl.operating_income + pl.total_other_income_expense
        
        # Tax Calculations
        pl.current_tax_expense = max(0, pl.income_before_tax * parameters.effective_tax_rate)
        pl.deferred_tax_expense = 0
        pl.tax_credits_applied = parameters.tax_credits
        pl.total_tax_expense = pl.current_tax_expense + pl.deferred_tax_expense - pl.tax_credits_applied
        pl.effective_tax_rate_actual = pl.total_tax_expense / pl.income_before_tax if pl.income_before_tax > 0 else 0
        
        # Net Income
        pl.net_income = pl.income_before_tax - pl.total_tax_expense
        pl.net_margin_percentage = pl.net_income / pl.total_revenue if pl.total_revenue > 0 else 0
        
        return pl
    
    def calculate_balance_sheet(self, parameters: CoreParameters, pl_statement: ProfitLossStatement, 
                               prior_balance_sheet: Optional[BalanceSheet] = None) -> BalanceSheet:
        ...
        prior_ppe = prior_balance_sheet.property_plant_equipment if prior_balance_sheet else 500000
        prior_accumulated_dep = prior_balance_sheet.accumulated_depreciation if prior_balance_sheet else 100000
        ...
        bs.accounts_receivable = (pl_statement.total_revenue * parameters.accounts_receivable_days) / 365
        bs.allowance_for_doubtful_accounts = bs.accounts_receivable * 0.02
        bs.net_accounts_receivable = bs.accounts_receivable - bs.allowance_for_doubtful_accounts
        ...
        bs.total_current_assets = (bs.cash + bs.cash_equivalents + bs.marketable_securities + 
                                 bs.net_accounts_receivable + bs.inventory + bs.prepaid_expenses + 
                                 bs.other_current_assets)
        
        # Non-Current Assets
        bs.property_plant_equipment = prior_ppe + (pl_statement.total_revenue * parameters.growth_capex_percentage)
        bs.accumulated_depreciation = prior_accumulated_dep + (bs.property_plant_equipment * parameters.depreciation_percentage)
        bs.net_property_plant_equipment = bs.property_plant_equipment - bs.accumulated_depreciation
        bs.intangible_assets = pl_statement.total_revenue * 0.1
        bs.accumulated_amortization = bs.intangible_assets * parameters.amortization_percentage
        ...
        bs.total_non_current_assets = (bs.net_property_plant_equipment + bs.net_intangible_assets + 
        ...
        # Current Liabilities
        bs.accounts_payable = (pl_statement.total_cogs * parameters.accounts_payable_days) / 365
        ...
        bs.total_current_liabilities = (bs.accounts_payable + bs.short_term_debt + bs.accrued_expenses + 
                                      bs.deferred_revenue + bs.income_taxes_payable + bs.other_current_liabilities)
        ...
        bs.total_non_current_liabilities = (bs.long_term_debt + bs.deferred_tax_liabilities + 
                                          bs.pension_liabilities + bs.other_long_term_liabilities)
        
        # Total Liabilities
        bs.total_liabilities = bs.total_current_liabilities + bs.total_non_current_liabilities
        
        # Equity
        bs.common_stock = 100000  # Fixed value
        bs.additional_paid_in_capital = 200000  # Fixed value
        bs.treasury_stock = 0
        bs.retained_earnings = bs.total_assets - bs.total_liabilities - bs.common_stock - bs.additional_paid_in_capital
        bs.other_equity = 0
        bs.total_equity = bs.common_stock + bs.additional_paid_in_capital + bs.retained_earnings + bs.other_equity - bs.treasury_stock
        
        # Balance Sheet Ratios
        bs.current_ratio = bs.total_current_assets / bs.total_current_liabilities if bs.total_current_liabilities > 0 else 0
        bs.quick_ratio = (bs.total_current_assets - bs.inventory) / bs.total_current_liabilities if bs.total_current_liabilities > 0 else 0
        bs.debt_to_equity_ratio = bs.total_liabilities / bs.total_equity if bs.total_equity > 0 else 0
        bs.debt_to_assets_ratio = bs.total_liabilities / bs.total_assets if bs.total_assets > 0 else 0
        bs.working_capital = bs.total_current_assets - bs.total_current_liabilities
        
        return bs
    
    def calculate_cash_flow(self, parameters: CoreParameters, pl_statement: ProfitLossStatement, 
                           balance_sheet: BalanceSheet, prior_balance_sheet: Optional[BalanceSheet] = None) -> CashFlowStatement:
        """Calculate comprehensive Cash Flow Statement"""
        cf = CashFlowStatement()
        
        # Operating Activities
        cf.net_income = pl_statement.net_income
        cf.depreciation = balance_sheet.accumulated_depreciation * parameters.depreciation_percentage
        cf.amortization = balance_sheet.accumulated_amortization * parameters.amortization_percentage
        ...
        cf.accounts_receivable_change = -balance_sheet.net_accounts_receivable * 0.1 if not prior_balance_sheet else -(balance_sheet.net_accounts_receivable - prior_balance_sheet.net_accounts_receivable)
        cf.inventory_change = -balance_sheet.inventory * 0.1 if not prior_balance_sheet else -(balance_sheet.inventory - prior_balance_sheet.inventory)
        cf.accounts_payable_change = balance_sheet.accounts_payable * 0.1 if not prior_balance_sheet else (balance_sheet.accounts_payable - prior_balance_sheet.accounts_payable)
        cf.accrued_expenses_change = balance_sheet.accrued_expenses * 0.1 if not prior_balance_sheet else (balance_sheet.accrued_expenses - prior_balance_sheet.accrued_expenses)
        cf.net_working_capital_change = cf.accounts_receivable_change + cf.inventory_change + cf.accounts_payable_change + cf.accrued_expenses_change
        
        cf.operating_cash_flow = (cf.net_income + cf.depreciation + cf.amortization + cf.stock_based_compensation + 
                                cf.deferred_taxes + cf.net_working_capital_change)
        
        # Investing Activities
        cf.capital_expenditures = -(pl_statement.total_revenue * (parameters.maintenance_capex_percentage + parameters.growth_capex_percentage))
        ...
        cf.investing_cash_flow = (cf.capital_expenditures + cf.software_development + cf.intangible_assets_investment + 
                                cf.marketable_securities_purchase + cf.marketable_securities_sale + cf.business_acquisitions)
        
        # Financing Activities
        cf.short_term_debt_issuance = balance_sheet.short_term_debt * 0.1 if not prior_balance_sheet else max(0, balance_sheet.short_term_debt - (prior_balance_sheet.short_term_debt if prior_balance_sheet else 0))
        cf.long_term_debt_issuance = balance_sheet.long_term_debt * 0.05 if not prior_balance_sheet else max(0, balance_sheet.long_term_debt - (prior_balance_sheet.long_term_debt if prior_balance_sheet else 0))
        cf.debt_repayment = -(balance_sheet.short_term_debt + balance_sheet.long_term_debt) * 0.02
        cf.common_stock_issuance = 0
        cf.stock_repurchase = 0
        cf.dividends_paid = -pl_statement.net_income * 0.2 if pl_statement.net_income > 0 else 0
        cf.financing_cash_flow = (cf.short_term_debt_issuance + cf.long_term_debt_issuance + cf.debt_repayment + 
                                cf.common_stock_issuance + cf.stock_repurchase + cf.dividends_paid)
        
        # Net Cash Flow
        cf.net_cash_flow = cf.operating_cash_flow + cf.investing_cash_flow + cf.financing_cash_flow
        cf.beginning_cash_balance = prior_balance_sheet.cash if prior_balance_sheet else 100000
        cf.ending_cash_balance = cf.beginning_cash_balance + cf.net_cash_flow
        
        # Cash Flow Metrics
        cf.operating_cash_flow_margin = cf.operating_cash_flow / pl_statement.total_revenue if pl_statement.total_revenue > 0 else 0
        cf.free_cash_flow = cf.operating_cash_flow + cf.capital_expenditures
        cf.free_cash_flow_margin = cf.free_cash_flow / pl_statement.total_revenue if pl_statement.total_revenue > 0 else 0
        cf.cash_conversion_cycle = parameters.accounts_receivable_days + parameters.inventory_days - parameters.accounts_payable_days
        
        return cf
    
    def calculate_dcf_valuation(self, parameters: CoreParameters, base_revenue: float = 1000000) -> DCFValuation:
        ...
        dcf.cost_of_debt = parameters.long_term_interest_rate * (1 - parameters.effective_tax_rate)
        dcf.tax_rate = parameters.effective_tax_rate
        dcf.debt_weight = 0.3  # Assume 30% debt
        dcf.equity_weight = 0.7  # Assume 70% equity
        dcf.wacc = (dcf.equity_weight * dcf.cost_of_equity) + (dcf.debt_weight * dcf.cost_of_debt)
        ...
        for year in range(parameters.projection_period_years):
            # Revenue projection
            projected_revenue = current_revenue * (1 + parameters.product_revenue_growth) ** (year + 1)
            ...
            # CapEx projection
            capex = projected_revenue * (parameters.maintenance_capex_percentage + parameters.growth_capex_percentage)
        ...
        terminal_fcf = dcf.projected_free_cash_flow[-1] * (1 + parameters.terminal_growth_rate)
        ...
        dcf.present_value_fcf = sum([fcf / ((1 + dcf.wacc) ** (i + 1)) for i, fcf in enumerate(dcf.projected_free_cash_flow)])
        dcf.present_value_terminal = dcf.terminal_value / ((1 + dcf.wacc) ** parameters.projection_period_years)
    ...
    def calculate_comprehensive_model(self, parameters: CoreParameters, base_revenue: float = 1000000, 
                                    prior_balance_sheet: Optional[BalanceSheet] = None) -> Dict[str, Any]:
        ...
        balance_sheet = self.calculate_balance_sheet(parameters, pl_statement, prior_balance_sheet)
        
        # Calculate Cash Flow
        cash_flow = self.calculate_cash_flow(parameters, pl_statement, balance_sheet, prior_balance_sheet)
    ...
    def create_scenario(self, scenario_name: str, parameters: CoreParameters, base_revenue: float = 1000000) -> Dict[str, Any]:
    ...
    def compare_scenarios(self, scenarios: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(scenarios) > 1:
            ...
            for metric in ['total_revenue', 'net_income', 'enterprise_value', 'value_per_share']:
                base_value = base_scenario[metric]
                variances = []
                
                for scenario_summary in comparison['scenarios'][1:]:
                    variance = ((scenario_summary[metric] - base_value) / base_value * 100) if base_value != 0 else 0
                    variances.append({
                        'scenario': scenario_summary['name'],
                        'variance_percent': variance,
                        'absolute_difference': scenario_summary[metric] - base_value
                    })
