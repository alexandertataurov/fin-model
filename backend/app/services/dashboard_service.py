...
class DashboardService:
    ...
    def __init__(self, db: Session):
        ...
        self.enhanced_charts = EnhancedChartMethods(
            self.chart_service, self.metrics_service
        )
    ...
    async def get_user_dashboard_data(
        ...
        try:
            ...
            key_metrics = await self._calculate_dashboard_key_metrics(
                statements, period
            )

            # Generate chart data using enhanced chart service
            chart_data = await self.enhanced_charts.generate_enhanced_dashboard_charts(
                statements, period.value
            )
            ...
            logging.exception(
                "Failed to build user dashboard data, falling back to demo data"
            )
            try:
                if fallback == "empty":
                    return DashboardData(
                ...
                logging.exception(
                    "Demo data generation failed; returning minimal dashboard response"
                )
                return DashboardData(
                    statements=[],
                    active_statement=None,
    ...
    async def get_key_metrics(
        self, user_id: int, statement_ids: List[int] = None
    ) -> Dict[str, Any]:
    ...
    async def _calculate_dashboard_key_metrics(
        self, statements: List[FinancialStatement], period: PeriodFilter
    ) -> Dict[str, Any]:
        ...
        if pl_statements:
            ...
            cogs = self._extract_metric_value(
        ...
        if bs_statements:
            ...
            current_assets = self._extract_metric_value(
                bs_data, ["current_assets", "total_current_assets"]
            )
            current_liabilities = self._extract_metric_value(
            ...
            total_debt = self._extract_metric_value(
                bs_data, ["total_debt", "total_liabilities"]
            )
            total_equity = self._extract_metric_value(
                bs_data, ["total_equity", "shareholders_equity"]
            )
        ...
        if cf_statements:
            ...
            operating_cf = self._extract_metric_value(
                cf_data, ["operating_cash_flow", "cash_from_operations"]
            )
            if operating_cf:
    ...
    async def _generate_revenue_trend_chart(
        self, pl_statements: List[FinancialStatement]
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_pl_data(self, statement: FinancialStatement) -> Dict[str, Any]:
        ...
        return {
            "revenue": {
                "total_revenue": self._extract_metric_value(
                    line_items, ["revenue", "total_revenue", "net_sales"]
                ),
                "product_revenue": self._extract_metric_value(
                    line_items, ["product_revenue", "goods_revenue"]
                ),
                "service_revenue": self._extract_metric_value(
                    line_items, ["service_revenue", "services_revenue"]
                ),
            },
            "expenses": {
                "cogs": self._extract_metric_value(
                    line_items, ["cogs", "cost_of_goods_sold"]
                ),
                "operating_expenses": self._extract_metric_value(
                    line_items, ["operating_expenses", "opex"]
                ),
                "interest_expense": self._extract_metric_value(
                    line_items, ["interest_expense"]
                ),
            },
            "profit": {
                "gross_profit": self._extract_metric_value(
                    line_items, ["gross_profit", "gross_income"]
                ),
                "operating_profit": self._extract_metric_value(
                    line_items, ["operating_profit", "ebit"]
                ),
                "net_income": self._extract_metric_value(
                    line_items, ["net_income", "net_profit"]
                ),
            },
        }

    def _calculate_pl_metrics(
        self, statement: FinancialStatement
    ) -> List[Dict[str, Any]]:
    ...
    async def _generate_profit_margins_chart(
        self, statement: FinancialStatement
    ) -> List[Dict[str, Any]]:
        ...
        if gross_profit:
            ...
            chart_data.append(
                {"period": "Gross Margin", "value": margin, "label": "Gross Margin"}
            )
        ...
        if net_income:
            ...
            chart_data.append(
                {"period": "Net Margin", "value": margin, "label": "Net Margin"}
            )
    ...
    def _extract_balance_sheet_data(
        self, statement: FinancialStatement
    ) -> Dict[str, Any]:
        ...
        return {
            "assets": {
                "current_assets": self._extract_metric_value(
                    line_items, ["current_assets", "total_current_assets"]
                ),
                "fixed_assets": self._extract_metric_value(
                    line_items, ["fixed_assets", "non_current_assets", "ppe"]
                ),
                "total_assets": self._extract_metric_value(
                    line_items, ["total_assets"]
                ),
            },
            "liabilities": {
                "current_liabilities": self._extract_metric_value(
                    line_items, ["current_liabilities", "total_current_liabilities"]
                ),
                "long_term_liabilities": self._extract_metric_value(
                    line_items, ["long_term_liabilities", "non_current_liabilities"]
                ),
                "total_liabilities": self._extract_metric_value(
                    line_items, ["total_liabilities"]
                ),
            },
            "equity": {
                "shareholders_equity": self._extract_metric_value(
                    line_items, ["shareholders_equity", "total_equity"]
                ),
                "retained_earnings": self._extract_metric_value(
                    line_items, ["retained_earnings"]
                ),
            },
        }
    ...
    async def _generate_asset_composition_chart(
        self, statement: FinancialStatement
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_cash_flow_data(self, statement: FinancialStatement) -> Dict[str, Any]:
        ...
        return {
            "operating": {
                "operating_cash_flow": self._extract_metric_value(
                    line_items, ["operating_cash_flow", "cash_from_operations"]
                ),
                "net_income": self._extract_metric_value(line_items, ["net_income"]),
                "depreciation": self._extract_metric_value(
                    line_items, ["depreciation", "depreciation_amortization"]
                ),
            },
            "investing": {
                "investing_cash_flow": self._extract_metric_value(
                    line_items, ["investing_cash_flow", "cash_from_investing"]
                ),
                "capex": self._extract_metric_value(
                    line_items, ["capital_expenditures", "capex"]
                ),
                "investments": self._extract_metric_value(
                    line_items, ["investments", "asset_purchases"]
                ),
            },
            "financing": {
                "financing_cash_flow": self._extract_metric_value(
                    line_items, ["financing_cash_flow", "cash_from_financing"]
                ),
                "debt_issuance": self._extract_metric_value(
                    line_items, ["debt_issuance", "borrowings"]
                ),
                "dividends": self._extract_metric_value(
                    line_items, ["dividends_paid", "dividends"]
                ),
            },
        }
            ...
            metrics.append(
                {
                    ...
                    "trend": "down"
                    if investing_cf < 0
                    else "up",  # Negative is normal for investing
                }
            )
    ...
    async def _generate_cash_flow_waterfall_chart(
        self, statement: FinancialStatement
    ) -> List[Dict[str, Any]]:
    ...
    async def _generate_operating_cf_trend_chart(
        self, statement: FinancialStatement
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_statement_quality_score(
        self, statement: FinancialStatement
    ) -> float:
    ...
    async def _get_demo_dashboard_data(
        self, user_id: int, period: PeriodFilter
    ) -> DashboardData:
        """Get demo dashboard data when no real data is available."""

        # Fallback to legacy service for demo data
        legacy_data = await self.legacy_service.get_overview_metrics(
            user_id, period.value
        )
    ...
    async def _calculate_cross_statement_metrics(
        self, statements: List[FinancialStatement]
    ) -> Dict[str, Any]:
        ...
        if pl_statements and bs_statements:
            ...
            net_income = self._extract_metric_value(
                latest_pl.line_items or {}, ["net_income", "net_profit"]
            )
            total_assets = self._extract_metric_value(
                latest_bs.line_items or {}, ["total_assets"]
            )
        ...
        if cf_statements and pl_statements:
            ...
            operating_cf = self._extract_metric_value(
                latest_cf.line_items or {}, ["operating_cash_flow"]
            )
            revenue = self._extract_metric_value(
                latest_pl.line_items or {}, ["revenue", "total_revenue"]
            )
    ...
    async def _generate_liability_composition_chart(
        self, statement: FinancialStatement
    ) -> List[Dict[str, Any]]:
    ...
    async def _generate_liquidity_ratios_chart(
        self, statement: FinancialStatement
    ) -> List[Dict[str, Any]]:
