...
class ReportService:
    ...
    def __init__(self, db: Session):
        ...
        self.default_templates = {
            ...
            ReportType.PROFIT_LOSS: {
                ...
                "sections": ["pl_metrics", "revenue_analysis", "expense_breakdown", "margins"],
                "layout": "detailed",
                "include_charts": True,
            ...
            ReportType.CASH_FLOW: {
                ...
                "sections": ["operating_cf", "investing_cf", "financing_cf", "waterfall"],
                "layout": "waterfall",
                "include_charts": True,
        ...
        try:
            ...
            if export_format == ExportFormat.PDF:
                ...
                file_path = self.excel_exporter.export_raw_data_csv(
                    raw_data,
                    filename=f"{export_record.name}.csv"
                )
            else:
            ...
            export_record.processing_duration_seconds = int(
                (export_record.processing_completed_at - export_record.processing_started_at).total_seconds()
            )
    ...
    async def _gather_financial_data(
        ...
        overview_data = await self.dashboard_service.get_overview_metrics(user_id, period, file_id)
        pl_data = await self.dashboard_service.get_pl_metrics(user_id, period, file_id)
        cf_data = await self.dashboard_service.get_cash_flow_metrics(user_id, period, file_id)
        bs_data = await self.dashboard_service.get_balance_sheet_metrics(user_id, period, file_id)
    ...
    def get_exports(
        ...
        return query.order_by(ReportExport.created_at.desc()).offset(offset).limit(limit).all()
    
    def get_export(self, export_id: int, user_id: int) -> Optional[ReportExport]:
        """Get a specific export."""
        return self.db.query(ReportExport).filter(
            and_(
    ...
    def delete_export(self, export_id: int, user_id: int) -> bool:
        """Delete an export."""
        export = self.db.query(ReportExport).filter(
            and_(
                ReportExport.id == export_id,
                ReportExport.created_by == user_id
            )
    ...
    def cleanup_expired_exports(self) -> int:
        ...
        now = datetime.utcnow()
        expired_exports = self.db.query(ReportExport).filter(
            and_(
    ...
    def get_export_summary(self, user_id: int) -> Dict[str, Any]:
        ...
        exports = self.db.query(ReportExport).filter(ReportExport.created_by == user_id).all()
        
        total_exports = len(exports)
        completed_exports = len([e for e in exports if e.status == ReportStatus.COMPLETED])
        failed_exports = len([e for e in exports if e.status == ReportStatus.FAILED])
        pending_exports = len([e for e in exports if e.status in [ReportStatus.PENDING, ReportStatus.PROCESSING]])
