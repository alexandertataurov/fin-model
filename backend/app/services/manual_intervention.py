...
class ManualInterventionService:
    ...
    def request_intervention(
        ...
        if not self._validate_admin_permissions(admin_user_id):
            raise HTTPException(
                status_code=403,
                detail="Insufficient permissions for intervention request",
            )
    ...
    async def execute_intervention(
        ...
        if not self._validate_admin_permissions(executing_admin_id):
            raise HTTPException(
                status_code=403,
                detail="Insufficient permissions for intervention execution",
            )

        if intervention_request.approval_required and not approved:
            raise HTTPException(
                status_code=403, detail="Intervention requires approval"
            )

        with SessionLocal() as db:
            file_record = (
                db.query(UploadedFile)
                .filter(UploadedFile.id == intervention_request.file_id)
                .first()
            )
            ...
            if (
                intervention_request.intervention_type
                == InterventionType.FORCE_REPROCESS
            ):
                result = await self._execute_force_reprocess(
                    file_record, intervention_request, executing_admin_id, db
                )
            elif (
                intervention_request.intervention_type
                == InterventionType.OVERRIDE_VALIDATION
            ):
                result = await self._execute_override_validation(
                    file_record, intervention_request, executing_admin_id, db
                )
            elif (
                intervention_request.intervention_type
                ...
                result = await self._execute_manual_data_entry(
                    file_record, intervention_request, executing_admin_id, db
                )
            elif (
                intervention_request.intervention_type
                == InterventionType.STATUS_OVERRIDE
            ):
                result = await self._execute_status_override(
                    file_record, intervention_request, executing_admin_id, db
                )
            elif (
                intervention_request.intervention_type
                ...
                result = await self._execute_emergency_recovery(
                    file_record, intervention_request, executing_admin_id, db
                )
            elif (
                intervention_request.intervention_type
                == InterventionType.DATA_CORRECTION
            ):
                result = await self._execute_data_correction(
                    file_record, intervention_request, executing_admin_id, db
                )
            elif (
                intervention_request.intervention_type
                == InterventionType.FILE_QUARANTINE
            ):
                result = await self._execute_file_quarantine(
                    file_record, intervention_request, executing_admin_id, db
                )
            else:
                raise HTTPException(
                    status_code=400,
                    detail=f"Unsupported intervention type: {intervention_request.intervention_type}",
                )

            # Log intervention execution
            self._log_intervention_execution(
                db, intervention_request, result, executing_admin_id
            )
    ...
    async def _execute_force_reprocess(
        ...
        try:
            ...
            task = process_uploaded_file.delay(
                file_record.id, request.parameters.get("processing_options", {})
            )
            ...
            return InterventionResult(
                ...
                audit_trail={"task_id": task.id, "parameters": request.parameters},
            )
    ...
    async def _execute_override_validation(
        ...
        try:
            ...
            # Override validation
            override_reason = request.parameters.get(
    ...
    async def _execute_manual_data_entry(
        ...
        try:
            ...
            if file_record.parsed_data:
                existing_data = (
                    json.loads(file_record.parsed_data)
                    if isinstance(file_record.parsed_data, str)
                    else file_record.parsed_data
                )
    ...
    async def _execute_status_override(
        ...
        try:
            ...
            actions_taken.append(
                f"Changed status from {original_status} to {new_status}"
            )
            ...
            return InterventionResult(
                ...
                intervention_timestamp=datetime.utcnow(),
                performed_by=admin_id,
                audit_trail={"status_change": f"{original_status} -> {new_status}"},
            )
    ...
    async def _execute_data_correction(
        self,
        file_record: UploadedFile,
        request: InterventionRequest,
        admin_id: int,
    ...
    async def _execute_file_quarantine(
        self,
        file_record: UploadedFile,
        request: InterventionRequest,
        admin_id: int,
    ...
    def _requires_approval(
        self, intervention_type: InterventionType, priority: InterventionPriority
    ) -> bool:
        ...
        return intervention_type in high_risk_interventions or priority in [
    ...
    def _log_intervention_execution(
        self,
        db: Session,
        request: InterventionRequest,
