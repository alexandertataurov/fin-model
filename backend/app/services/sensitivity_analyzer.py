...
from app.models.parameter import Parameter, ParameterValue, Scenario, SensitivityAnalysis
...
class SensitivityAnalyzer:
    ...
    async def run_tornado_analysis(
        ...
        target_param = self.db.query(Parameter).filter(
            Parameter.id == target_parameter_id
        ).first()
        ...
        for config in input_parameters:
            param = self.db.query(Parameter).filter(
                Parameter.id == config.parameter_id
            ).first()
        ...
        analysis = SensitivityAnalysis(
            ...
            summary_statistics={
                ...
                "highest_impact": sensitivity_results[0].parameter_name if sensitivity_results else None,
                "total_variance_explained": sum(abs(r.sensitivity_coefficient) for r in sensitivity_results[:5])
            },
            status="completed",
    ...
    async def run_monte_carlo_simulation(
        ...
        for j, config in enumerate(input_parameters):
            ...
            parameter_correlations[config.parameter_id] = float(correlation) if not np.isnan(correlation) else 0.0
        ...
        for config in input_parameters:
            ...
            for variation_pct in variation_percentages:
                ...
                if target_result.error is None:
                    ...
                    target_change_pct = ((target_result.value - base_value) / base_value * 100) if base_value != 0 else 0
    ...
    async def _calculate_parameter_sensitivity(
        ...
        sensitivity_coefficient = (target_range / param_range) if param_range != 0 else 0
    ...
    async def _get_parameter_value(self, parameter: Parameter, scenario_id: int) -> float:
        """
                    ...
                    "variations": [
                        {
                            ...
                            "target_change_pct": data["variation_results"].get(var_pct, 0)
                        }
