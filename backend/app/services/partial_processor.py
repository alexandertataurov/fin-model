...
class PartialProcessor:
    ...
    def process_with_issues(self, file_path: str, validation_result: FileValidationResult) -> PartialProcessingResult:
        """
        ...
        try:
            ...
            for sheet_name in workbook.sheetnames:
                try:
                    ...
                    if sheet_result["success"]:
                        ...
                        extracted_data["partial_financial_statements"][sheet_name] = sheet_result["data"]
            ...
            if sheets_processed:
                metrics_result = self._extract_partial_metrics(workbook, sheets_processed, issues)
                extracted_data["recoverable_metrics"] = metrics_result["metrics"]
                recovery_actions.extend(metrics_result["recovery_actions"])
            
            # Calculate completion percentage
            completion_percentage = (len(sheets_processed) / total_sheets) * 100 if total_sheets > 0 else 0
            
            # Adjust completion based on data quality
            if extracted_data["recoverable_metrics"]:
                completion_percentage = min(100, completion_percentage + 20)  # Bonus for metrics
            
            # Generate recommendations
            recommendations = self._generate_recovery_recommendations(issues, sheets_failed)
            ...
            return PartialProcessingResult(
                ...
                recommendations=["File may be corrupted", "Try re-uploading the file", "Contact support if issue persists"]
            )
    
    def _process_sheet_with_recovery(self, workbook, sheet_name: str, global_issues: List[ProcessingIssue]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                numeric_data = self._extract_numeric_data_with_recovery(sheet, result["issues"])
                sheet_data["extracted_values"] = numeric_data["values"]
                result["recovery_actions"].extend(numeric_data["recovery_actions"])
                
                # Extract text data and labels
                text_data = self._extract_text_data_with_recovery(sheet, result["issues"])
                ...
                formula_data = self._extract_formulas_with_recovery(sheet, result["issues"])
                ...
                result["issues"].append(ProcessingIssue(
                    issue_type=ProcessingIssueType.PARSING_ERROR,
                ...
                if basic_data:
                    ...
                    result["success"] = True
                    result["recovery_actions"].append("Used basic cell-by-cell recovery")
    ...
    def _extract_numeric_data_with_recovery(self, sheet, issues: List[ProcessingIssue]) -> Dict[str, Any]:
        ...
        if recovery_count > 0:
            result["recovery_actions"].append(f"Recovered {recovery_count} numeric values from text")
        
        return result
    
    def _extract_text_data_with_recovery(self, sheet, issues: List[ProcessingIssue]) -> Dict[str, Any]:
    ...
    def _extract_formulas_with_recovery(self, sheet, issues: List[ProcessingIssue]) -> Dict[str, Any]:
                    ...
                    if cell.data_type == 'f':  # Formula cell
                        try:
                            formulas.append({
                                ...
                                "formula": cell.value,
                                "calculated_value": cell.value if hasattr(cell, 'value') else None
                            })
                        except Exception as e:
                            issues.append(ProcessingIssue(
    ...
    def _basic_cell_recovery(self, sheet, issues: List[ProcessingIssue]) -> Optional[Dict[str, Any]]:
    ...
    def _extract_partial_metrics(self, workbook, processed_sheets: List[str], issues: List[ProcessingIssue]) -> Dict[str, Any]:
        ...
        try:
            ...
            partial_extraction = self.financial_extractor.extract_comprehensive_data(workbook.path)
            ...
            if partial_extraction and not partial_extraction.get("error"):
                ...
                result["metrics"] = {
                    ...
                    "extraction_confidence": len(filtered_metrics) / max(len(partial_extraction.get("financial_metrics", [])), 1),
                    "sheets_contributing": processed_sheets
                }
                
                result["recovery_actions"].append(f"Extracted {len(filtered_metrics)} financial metrics from {len(processed_sheets)} sheets")
    ...
    def _identify_metric_sheet(self, metric: Dict[str, Any], processed_sheets: List[str]) -> Optional[str]:
    ...
    def _generate_recovery_recommendations(self, issues: List[ProcessingIssue], failed_sheets: List[str]) -> List[str]:
        """Generate recommendations based on processing issues."""
        recommendations = []
        
        # Analyze issue patterns
        critical_issues = [i for i in issues if i.severity == ProcessingSeverity.CRITICAL]
        high_issues = [i for i in issues if i.severity == ProcessingSeverity.HIGH]
        
        if critical_issues:
            recommendations.append("File has critical issues that prevent complete processing")
            recommendations.append("Consider manually fixing the file before re-uploading")
        
        if failed_sheets:
            recommendations.append(f"Could not process {len(failed_sheets)} sheets: {', '.join(failed_sheets[:3])}")
        ...
        if ProcessingIssueType.FORMULA_ERROR in issue_types:
            recommendations.append("Some formulas could not be evaluated - check for circular references or missing data")
        
        if ProcessingIssueType.MISSING_DATA in issue_types:
            recommendations.append("Fill in missing data fields for more complete analysis")
        
        if ProcessingIssueType.INVALID_FORMAT in issue_types:
            recommendations.append("Review data formatting - some values may not be recognized as numbers")
        
        # General recommendations
        if not recommendations:
            recommendations.append("Processing completed successfully with partial data extraction")
        
        recommendations.append("Review the extracted data to ensure it meets your requirements")
