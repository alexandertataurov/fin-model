...
class ExcelParser:
    ...
    def parse_excel_file(self, file_path: str) -> ParsedFileData:
        ...
        try:
            ...
            validation_result = self._validate_financial_statements(
                sheets_info, workbook
            )
            ...
            if validation_result.is_valid:
                ...
                comprehensive_extraction = (
                    self.financial_extractor.extract_comprehensive_data(file_path)
                )
            ...
            return ParsedFileData(
                ...
                validation_summary=FileValidationResult(
                    is_valid=False, errors=[f"Failed to parse Excel file: {str(e)}"]
                ),
            )
    ...
    def _detect_headers(self, sheet, min_row: int, min_col: int, max_col: int) -> bool:
        ...
        for col in range(
            min_col, min_col + min(max_col - min_col + 1, 10)
    ...
    def _validate_financial_statements(
        self, sheets_info: List[ExcelSheetInfo], workbook
    ) -> FileValidationResult:
        ...
        for sheet in sheets_info:
            ...
            if sheet_type:
                financial_sheets.append(
                    {"name": sheet.name, "type": sheet_type, "columns": sheet.columns}
                )
    ...
    def _identify_sheet_type(self, sheet_info: ExcelSheetInfo) -> Optional[str]:
        ...
        if any(
            indicator in name_lower for indicator in ["p&l", "pnl", "income", "profit"]
        ):
            return "pnl"
        if (
            sum(
    ...
    def _validate_sheet_structure(
        ...
        for required_col in required_columns:
            ...
            if mapped_col:
                ...
                errors.append(
                    f"Required column '{required_col}' not found in {fs_sheet['name']} sheet"
                )
    ...
    def _find_matching_column(
        self, required_col: str, available_columns: List[str]
    ) -> Optional[str]:
        ...
        for row_idx in range(2, min(12, sheet.max_row + 1)):
            ...
            if cell_value is not None:
                ...
                if isinstance(cell_value, (int, float)):
                    numeric_count += 1
                elif isinstance(cell_value, str) and self._is_numeric_string(
                    cell_value
                ):
                    numeric_count += 1
    ...
    def _extract_financial_statements(
        self, workbook, validation_result: FileValidationResult
    ) -> Dict[str, Any]:
    ...
    def _extract_sheet_data(
        self, sheet, sheet_type: str, column_mapping: Dict[str, str]
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_key_metrics(
        self, workbook, financial_statements: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        pnl_sheets = [
            name for name, data in financial_statements.items() if data["type"] == "pnl"
        ]
        if pnl_sheets:
            pnl_data = financial_statements[pnl_sheets[0]]["data"]
            metrics["revenue"] = self._find_metric_value(
                pnl_data, ["revenue", "sales", "income"]
            )
            metrics["expenses"] = self._find_metric_value(
                pnl_data, ["expense", "cost", "operating"]
            )
            metrics["profit"] = self._find_metric_value(
                pnl_data, ["profit", "net income", "earnings"]
            )
        ...
        if bs_sheets:
            ...
            metrics["total_assets"] = self._find_metric_value(
                bs_data, ["total assets", "assets"]
            )
            metrics["total_liabilities"] = self._find_metric_value(
                bs_data, ["total liabilities", "liabilities"]
            )
            metrics["equity"] = self._find_metric_value(
                bs_data, ["equity", "shareholders equity"]
            )

        return metrics

    def _find_metric_value(
        self, data: List[Dict[str, Any]], search_terms: List[str]
    ) -> Optional[float]:
        ...
        for row_idx in range(1, min(50, sheet.max_row + 1)):  # Check first 50 rows
            for col_idx in range(
                ...
                if (
                    ...
                    and isinstance(key_cell.value, str)
