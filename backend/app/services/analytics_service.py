...
class AnalyticsService:
    ...
    def get_processing_overview(self, days: int = 30) -> Dict[str, Any]:
        ...
        total_files = self.db.query(func.count(UploadedFile.id)).filter(
            UploadedFile.created_at >= start_date
        ).scalar()
        
        completed_files = self.db.query(func.count(UploadedFile.id)).filter(
            and_(
                UploadedFile.created_at >= start_date,
        ...
        processing_files = self.db.query(func.count(UploadedFile.id)).filter(
            and_(
                UploadedFile.created_at >= start_date,
        ...
        # Convert to minutes
        avg_processing_minutes = round(avg_processing_time / 60, 2) if avg_processing_time else 0
        
        # Total file size processed
        total_size = self.db.query(func.sum(UploadedFile.file_size)).filter(
            UploadedFile.created_at >= start_date
        ).scalar() or 0
    ...
    def get_daily_processing_stats(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        daily_stats = self.db.query(
            ...
            func.count(UploadedFile.id).filter(UploadedFile.status == FileStatus.COMPLETED).label('completed'),
            func.count(UploadedFile.id).filter(UploadedFile.status == FileStatus.FAILED).label('failed'),
            func.sum(UploadedFile.file_size).label('total_size')
        ).filter(
            func.date(UploadedFile.created_at) >= start_date
        ).group_by(
            func.date(UploadedFile.created_at)
        ).order_by(
            func.date(UploadedFile.created_at)
        ).all()
        ...
        return [
            {
                ...
                "success_rate": round((stat.completed or 0) / stat.total_files * 100, 1) if stat.total_files > 0 else 0,
            ...
            "distribution": [
                {
                    ...
                    "count": ft.count,
                    "percentage": round(ft.count / total_files * 100, 1) if total_files > 0 else 0,
    ...
    def get_user_activity_stats(self, days: int = 30, limit: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        user_stats = self.db.query(
            ...
            func.count(UploadedFile.id).filter(UploadedFile.status == FileStatus.COMPLETED).label('successful_uploads'),
            func.sum(UploadedFile.file_size).label('total_size'),
            func.max(UploadedFile.created_at).label('last_upload')
        ).join(
            UploadedFile, User.id == UploadedFile.uploaded_by_id
        ).filter(
            UploadedFile.created_at >= start_date
        ).group_by(
            User.id, User.username, User.email
        ).order_by(
            func.count(UploadedFile.id).desc()
        ).limit(limit).all()
        ...
        return [
            {
                ...
                "success_rate": round((stat.successful_uploads or 0) / stat.total_uploads * 100, 1) if stat.total_uploads > 0 else 0,
                "total_size_mb": round((stat.total_size or 0) / (1024 * 1024), 2),
    ...
    def get_error_analysis(self, days: int = 30) -> Dict[str, Any]:
        ...
        # Get failed files
        failed_files = self.db.query(UploadedFile).filter(
            and_(
                UploadedFile.created_at >= start_date,
                UploadedFile.status == FileStatus.FAILED
            )
        ...
        error_logs = self.db.query(ProcessingLog).filter(
            and_(
        ...
        return {
            ...
            "processing_error_categories": [
                {"category": category, "count": count}
                for category, count in sorted(log_error_categories.items(), key=lambda x: x[1], reverse=True)
            ],
            "period": f"{days} days"
        }
    ...
    def get_dashboard_summary(self, days: int = 7) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_summary": {
                "avg_processing_time": performance["processing_time_stats"]["average_minutes"],
    ...
    def _categorize_error(self, error_message: str) -> str:
        ...
        if any(keyword in error_lower for keyword in ['file not found', 'path', 'missing']):
            return "File Access Error"
        elif any(keyword in error_lower for keyword in ['format', 'excel', 'csv', 'corrupt']):
            return "File Format Error"
        elif any(keyword in error_lower for keyword in ['size', 'large', 'limit']):
            return "File Size Error"
        elif any(keyword in error_lower for keyword in ['validation', 'template', 'structure']):
