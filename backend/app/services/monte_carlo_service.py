...
class MonteCarloService:
    ...
    async def run_monte_carlo(
        self, simulation_id: str, user_id: Optional[int] = None
    ) -> SimulationResult:
        """
        ...
        try:
            ...
            risk_metrics = self._calculate_risk_metrics(
                results, simulation.output_metrics
            )
            ...
            metric_risk.update(
                {
                    ...
                    "downside_deviation": float(
                        np.std(values_array[values_array < mean_value])
                    ),
                }
            )
    ...
    async def _generate_correlated_samples(
        ...
        correlated_samples = np.random.multivariate_normal(
            mean=np.zeros(n_params), cov=correlation_matrix, size=simulation.iterations
        )
    ...
    async def _execute_simulation(
        self, simulation: MonteCarloSimulation, parameter_samples: Dict[int, np.ndarray]
    ) -> Dict[str, List[float]]:
    ...
    def _calculate_risk_metrics(
        self, results: Dict[str, List[float]], output_metrics: List[str]
    ) -> Dict[str, float]:
