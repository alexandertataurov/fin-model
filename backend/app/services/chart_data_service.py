...
class ChartDataService:
    ...
    def create_cash_flow_trend_chart(
        ...
        for i, flow_type in enumerate(flow_types):
            ...
            for stmt in cf_statements:
                line_items = stmt.line_items or {}
                flow_value = self._extract_numeric_value(
                    line_items,
                    [
                        flow_type,
                        flow_type.replace("_", " "),
                ...
                if flow_value is not None:
                    period_label = self._format_period_label(
                        stmt.period_start, TimeGranularity.QUARTERLY
                    )
                    flow_data.append(
                        ChartDataPoint(
                            x=period_label,
        ...
        for i, metric in enumerate(metrics):
            ...
            for stmt in statements:
                ...
                value = self._extract_numeric_value(
                    line_items, [metric, metric.replace("_", " ")]
                )
    ...
    def create_cash_flow_waterfall_chart(
        ...
        if starting_cash is None:
            starting_cash = (
                self._extract_numeric_value(
                    line_items, ["beginning_cash", "opening_cash_balance"]
                )
                or 100000
            )  # Default fallback
        ...
        series = ChartSeries(
            name="Cash Flow", data=waterfall_data, type=ChartType.WATERFALL
        )
    ...
    def create_liquidity_ratios_chart(
        ...
        for i, (ratio_key, ratio_name) in enumerate(ratio_types):
            ...
            for stmt in bs_statements:
                ...
                # Calculate ratios
                current_assets = self._extract_numeric_value(
                    line_items, ["current_assets"]
                )
                current_liabilities = self._extract_numeric_value(
    ...
    def _format_period_label(
        self, date_value: datetime, granularity: TimeGranularity
    ) -> str:
                ...
                series_config.append(
                    {
                        ...
                        "name": series.name,
                        "color": series.color
                        or self.color_palette[i % len(self.color_palette)],
