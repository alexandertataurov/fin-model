...
class Parameter(Base):
    ...
    parameter_type = Column(
        ...
        nullable=False,
        index=True,
        default=ParameterType.GROWTH_RATE.value,
    )
    category = Column(
        String(50),
        nullable=False,
        index=True,
        default=ParameterCategory.ASSUMPTIONS.value,
    )
    sensitivity_level = Column(
        String(20), nullable=False, default=SensitivityLevel.MEDIUM
    )
    ...
    # Grouping
    group_id = Column(
        String(50), ForeignKey("parameter_groups.id"), nullable=True
    )
    ...
    parameter_values = relationship(
        "ParameterValue", back_populates="parameter"
    )
