...
from app.models.financial import (
    FinancialStatement, Metric, TimeSeries, Calculation, 
    Template, FileVersion, DataSource
)
...
class FinancialStatementRepository(BaseRepository[FinancialStatement]):
    ...
    def get_by_unique_field(self, field_name: str, value: Any) -> Optional[FinancialStatement]:
...
class MetricRepository(BaseRepository[Metric]):
    ...
    def get_metric_trends(
        ...
        ).order_by(desc(self.model.period_start)).limit(limit).all()
    ...
    def get_dashboard_metrics(self, scenario_id: int) -> Dict[str, List[Metric]]:
        ...
        metrics = self.db.query(self.model).filter(
            self.model.scenario_id == scenario_id
        ).order_by(self.model.metric_category, self.model.period_start).all()
...
class TimeSeriesRepository(BaseRepository[TimeSeries]):
    ...
    def bulk_upsert_series(self, scenario_id: int, data_type: str, series_data: List[Dict[str, Any]]) -> int:
...
class CalculationRepository(BaseRepository[Calculation]):
    ...
    def get_execution_order(self, scenario_id: int) -> List[Calculation]:
        ...
        ).order_by(self.model.execution_order).all()
    ...
    def update_execution_stats(self, calculation_id: int, execution_time_ms: int) -> bool:
...
class TemplateRepository(BaseRepository[Template]):
    ...
    def get_active_templates(self, template_type: Optional[str] = None) -> List[Template]:
...
class FileVersionRepository(BaseRepository[FileVersion]):
    ...
    def get_file_versions(self, file_id: int) -> List[FileVersion]:
        ...
        ).order_by(desc(self.model.version_number)).all()
...
class DataSourceRepository(BaseRepository[DataSource]):
    ...
    def get_by_type(self, source_type: str) -> List[DataSource]:
        ...
        ).order_by(self.model.source_name).all()
    ...
    def update_quality_metrics(self, source_id: int, quality_metrics: Dict[str, Any]) -> bool:
