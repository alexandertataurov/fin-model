...
@celery_app.task(bind=True, base=DatabaseTask, name='app.tasks.file_processing.process_uploaded_file')
def process_uploaded_file(self, db: Session, file_id: int, processing_options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        file_service.log_processing_step(file_id, "parsing", "Starting file parsing", "info")
        ...
        self.update_state(
            state='PROGRESS',
            meta={'current': 70, 'total': 100, 'status': 'Processing financial data...'}
        )
        ...
        file_service.update_file_status(
            file_id, 
            FileStatus.FAILED,
            validation_errors=json.dumps({"error": error_message, "traceback": error_details}),
            is_valid=False
        )
...
@celery_app.task(bind=True, base=DatabaseTask, name='app.tasks.file_processing.reprocess_file')
def reprocess_file(self, db: Session, file_id: int, processing_options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
...
@celery_app.task(bind=True, base=DatabaseTask, name='app.tasks.file_processing.cleanup_old_files')
def cleanup_old_files(self, db: Session, days_old: int = 30) -> Dict[str, Any]:
    ...
    old_files = db.query(UploadedFile).filter(
        ...
        UploadedFile.status.in_([FileStatus.COMPLETED, FileStatus.FAILED, FileStatus.CANCELLED])
    ).all()
