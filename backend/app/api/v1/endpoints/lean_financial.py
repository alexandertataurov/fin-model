...
class ParameterUpdateRequest(BaseModel):
    ...
    base_revenue: Optional[float] = Field(
        1000000, description="Base revenue for calculations"
    )
...
class ScenarioCreateRequest(BaseModel):
    ...
    parameters: Dict[str, float] = Field(
        ..., description="Parameter values for scenario"
    )
    base_revenue: Optional[float] = Field(
        1000000, description="Base revenue for calculations"
    )
...
class SensitivityAnalysisRequest(BaseModel):
    ...
    variation_percent: Optional[float] = Field(
        0.20, description="Variation percentage for sensitivity"
    )
...
async def calculate_individual_statement(
    ...
    try:
        ...
        if statement_type == "profit_loss":
            ...
            balance_sheet = engine.calculate_balance_sheet(
                core_parameters, pl_statement
            )
            result = engine.calculate_cash_flow(
                core_parameters, pl_statement, balance_sheet
            )
        elif statement_type == "dcf_valuation":
            result = engine.calculate_dcf_valuation(
                core_parameters, request.base_revenue
            )
...
async def create_scenario(
    ...
    try:
        ...
        # Create scenario
        scenario_result = engine.create_scenario(
            request.scenario_name, core_parameters, request.base_revenue
        )
