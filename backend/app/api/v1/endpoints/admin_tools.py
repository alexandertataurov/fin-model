...
from app.services.manual_intervention import (
    ManualInterventionService, InterventionType, InterventionPriority
)
...
def get_recovery_options(
    ...
    """
    try:
        recovery_actions = recovery_service.analyze_recovery_options(file_id, current_user.id)
...
async def execute_recovery(
    ...
    try:
        ...
        recovery_actions = recovery_service.analyze_recovery_options(request.file_id, current_user.id)
...
def analyze_partial_processing(
    ...
    try:
        ...
        file_record = db.query(UploadedFile).filter(
            and_(UploadedFile.id == request.file_id, UploadedFile.uploaded_by_id == current_user.id)
        ).first()
        ...
        result = partial_processor.process_with_issues(file_record.file_path, validation_result)
...
def execute_partial_processing(
    ...
    try:
        ...
        validation_result = type('obj', (object,), {
            'is_valid': False,
            'errors': json.loads(file_record.validation_errors).get('errors', []) if file_record.validation_errors else [],
            'warnings': json.loads(file_record.validation_errors).get('warnings', []) if file_record.validation_errors else []
        })()
...
def get_system_stats(
    ...
    try:
        ...
        file_stats = db.query(
            ...
            func.count(UploadedFile.id).filter(UploadedFile.status == 'completed').label('completed_files'),
            func.count(UploadedFile.id).filter(UploadedFile.status == 'failed').label('failed_files'),
            func.count(UploadedFile.id).filter(UploadedFile.status == 'processing').label('processing_files')
        ).first()
        ...
        return {
            ...
            "file_statistics": {
                ...
                "success_rate": round((file_stats.completed_files or 0) / max(file_stats.total_files or 1, 1) * 100, 1)
            },
