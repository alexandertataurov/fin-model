...
def get_docs_path():
    ...
    possible_paths = [
        Path("/app/docs/redocs"),  # Railway deployment
        Path("docs/redocs"),       # Local development
...
async def docs_landing():
    ...
    if Path("/app").exists():
        debug_info["app_dir_contents"] = [str(p) for p in Path("/app").iterdir()][:10]  # Limit to first 10
...
async def docs_redoc():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Error reading ReDoc file: {str(e)}"
        )
...
async def docs_swagger():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Error reading Swagger file: {str(e)}"
        )
