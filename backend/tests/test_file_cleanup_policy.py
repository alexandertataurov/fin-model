...
def make_file(
    created_at, file_type="excel", status=FileStatus.COMPLETED, size=1024 * 1024
):
    return SimpleNamespace(
        file_size=size, file_type=file_type, status=status, created_at=created_at
    )
...
def test_policy_size_and_user_tier():
    policy = FileRetentionPolicy(
        "p", "d", 1, size_threshold_mb=2, user_tier_specific="premium"
    )
