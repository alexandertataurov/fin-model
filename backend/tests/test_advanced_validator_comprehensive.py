...
class TestAdvancedValidator:
    ...
    def test_validate_balance_sheet_template_success(self, validator, sample_balance_sheet_data):
        """Test successful balance sheet template validation"""
        result = validator.validate_template(sample_balance_sheet_data, TemplateType.BALANCE_SHEET)
    ...
    def test_validate_cash_flow_template_success(self, validator, sample_cash_flow_data):
        """Test successful cash flow template validation"""
        result = validator.validate_template(sample_cash_flow_data, TemplateType.CASH_FLOW)
    ...
    def test_detect_column_mappings(self, validator, sample_pl_data):
        ...
        mappings = validator._detect_column_mappings(sheet_data, TemplateType.PROFIT_LOSS)
        
        assert len(mappings) > 0
        assert all(isinstance(mapping, ColumnMapping) for mapping in mappings)
        assert all(mapping.confidence >= 0.0 and mapping.confidence <= 1.0 for mapping in mappings)

    def test_identify_financial_sections_pl(self, validator, sample_pl_data):
        """Test financial section identification for P&L"""
        sheet_data = sample_pl_data["sheets"][0]
        sections = validator._identify_financial_sections(sheet_data, TemplateType.PROFIT_LOSS)
    ...
    def test_check_calculation_consistency(self, validator, sample_pl_data):
        ...
        sheet_data = sample_pl_data["sheets"][0]
        errors = validator._check_calculation_consistency(sheet_data, TemplateType.PROFIT_LOSS)
    ...
    def test_auto_detect_template_type(self, validator, sample_pl_data, sample_balance_sheet_data):
...
class TestTemplateValidationResult:
    ...
    def test_validation_result_creation(self):
        ...
        result = TemplateValidationResult(
            ...
            detected_columns=[
                ColumnMapping(0, "A", "Account", "Account", 1.0, "text")
            ],
...
class TestAdvancedValidatorIntegration:
    ...
    def test_comprehensive_pl_validation(self):
            ...
            "sheets": [
                {
                    ...
                    "type": "financial",
                    "headers": ["Line Item", "Current Year", "Prior Year", "Budget", "Variance"],
                    "data": [
        ...
        result = validator.validate_template(comprehensive_pl_data, TemplateType.PROFIT_LOSS)
    ...
    def test_error_recovery_and_suggestions(self):
        ...
        assert any(keyword in suggestions_text for keyword in ["add", "fix", "correct", "include"])