...
def test_permission_checker():
    ...
    analyst_permissions = PermissionChecker.get_user_permissions([RoleType.ANALYST.value])
    ...
    assert PermissionChecker.has_permission([RoleType.ADMIN.value], Permission.USER_CREATE)
    assert not PermissionChecker.has_permission([RoleType.VIEWER.value], Permission.USER_CREATE)


def test_resource_access():
    """Test resource access control."""
    # Test permission checking
    assert PermissionChecker.has_permission([RoleType.ADMIN.value], Permission.MODEL_CREATE)
    assert PermissionChecker.has_permission([RoleType.ANALYST.value], Permission.MODEL_READ)
    assert PermissionChecker.has_permission([RoleType.VIEWER.value], Permission.DATA_READ)
    
    # Test role hierarchy
    assert PermissionChecker.has_any_permission([RoleType.ADMIN.value], [Permission.USER_READ, Permission.USER_CREATE])
    assert not PermissionChecker.has_any_permission([RoleType.VIEWER.value], [Permission.USER_CREATE, Permission.USER_DELETE])
...
def test_multiple_roles():
    ...
    combined_perms = PermissionChecker.get_user_permissions([RoleType.ANALYST.value, RoleType.VIEWER.value])
    analyst_only_perms = PermissionChecker.get_user_permissions([RoleType.ANALYST.value])
