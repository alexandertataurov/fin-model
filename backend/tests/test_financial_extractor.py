...
class TestFinancialExtractor:
    ...
    def test_sheet_name_matching(self, extractor):
        ...
        test_cases = [
            ...
            {
                "sheets": [
                    {"name": "Cash Flow Statement", "data": [["Operating CF", 2000]]}
                ],
...
class TestFinancialExtractorIntegration:
    ...
    def test_comprehensive_extraction(self):
        ...
        revenue_item = next(
            item for item in income_stmt if item["account"] == "Total Revenue"
        )
        assert revenue_item["value"] == 850000

        balance_sheet = result["balance_sheet"]
        cash_item = next(
            item for item in balance_sheet if item["account"] == "Cash & Equivalents"
        )
