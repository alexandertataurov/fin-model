...
def test_compare_scenarios_with_filter():
    ...
    base_values = [ParameterValue(parameter_id=1, value=1), ParameterValue(parameter_id=2, value=2)]
    comp_values = [ParameterValue(parameter_id=1, value=1.5), ParameterValue(parameter_id=2, value=2)]
    db.query.return_value.filter.return_value.all.side_effect = [base_values, comp_values]
