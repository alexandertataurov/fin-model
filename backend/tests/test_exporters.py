...
def test_excel_exporter_creates_file(tmp_path):
    ...
    data = {
        "summary": {"metrics": {"revenue": 1000}},
        "pl": {"metrics": {"profit": 200}},
    }
...
def test_export_raw_data_csv(tmp_path):
    ...
    path = exporter.export_raw_data_csv(
        data, filename="out.csv", include_metadata=False
    )
    assert os.path.exists(path)
    with open(path) as f:
...
def test_pdf_format_currency_and_export_chart(tmp_path):
    ...
    img_path = generator.export_chart_as_image(
        chart_data, chart_type="line", filename="chart.png"
    )
