...
class TestAuthService:
    ...
    def test_create_user_success(self, mock_token, mock_hash, auth_service, mock_db, sample_role):
        mock_hash.return_value = "$2b$12$hashed_password"
        mock_token.return_value = "verification_token"
        mock_db.query.return_value.filter.return_value.first.side_effect = [None, None, sample_role]
    ...
    def test_create_user_full_name_construction(self, auth_service, mock_db, sample_role):
        with patch('app.services.auth_service.get_password_hash') as mock_hash, \
             patch('app.services.auth_service.generate_secure_token') as mock_token:
