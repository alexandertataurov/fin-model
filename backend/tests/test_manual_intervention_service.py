...
def test_requires_approval_high_risk():
    ...
    assert svc._requires_approval(
        InterventionType.MANUAL_DATA_ENTRY, InterventionPriority.NORMAL
    )
    assert svc._requires_approval(
        InterventionType.STATUS_OVERRIDE, InterventionPriority.EMERGENCY
    )
...
def test_validate_admin_permissions_true():
    ...
    with patch("app.services.manual_intervention.SessionLocal") as session:
        session.return_value.__enter__.return_value.query.return_value.filter.return_value.first.return_value = (
            user
        )
