...
class TestScenarioManager:
    ...
    def sample_parameter_value(self):
        return ParameterValue(
            id=1, parameter_id=1, scenario_id=1, value=0.08, created_by_id=1
        )

    @pytest.mark.asyncio
    async def test_create_scenario_success(
        self, scenario_manager, mock_db, sample_file
    ):
        ...
        with patch.object(
            scenario_manager, "_generate_version_number", return_value=1
        ) as mock_version, patch.object(
            scenario_manager, "_initialize_parameter_values"
        ) as mock_init:
    ...
    @pytest.mark.asyncio
    async def test_create_scenario_with_parent(
        self, scenario_manager, mock_db, sample_file, sample_scenario
    ):
        ...
        with patch.object(
            scenario_manager, "_generate_version_number", return_value=2
        ) as mock_version, patch.object(
            scenario_manager, "_copy_parameter_values"
        ) as mock_copy:
            result = await scenario_manager.create_scenario(
                name="Child Scenario",
    ...
    @pytest.mark.asyncio
    async def test_create_scenario_parent_not_found(
        self, scenario_manager, mock_db, sample_file
    ):
    ...
    @pytest.mark.asyncio
    async def test_clone_scenario_success(
        self, scenario_manager, mock_db, sample_scenario
    ):
    ...
    def test_update_parameter_value(
        self, scenario_manager, mock_db, sample_parameter_value
    ):
        ...
        result = scenario_manager.update_parameter_value(
            scenario_id=1, parameter_id=1, new_value=0.10, user_id=1
        )
    ...
    def test_update_parameter_value_not_found(self, scenario_manager, mock_db):
        ...
        result = scenario_manager.update_parameter_value(
            scenario_id=999, parameter_id=999, new_value=0.10, user_id=1
        )

        assert result is None

    @pytest.mark.asyncio
    async def test_calculate_scenario_results(
        self, scenario_manager, mock_db, sample_scenario
    ):
        ...
        with patch.object(
            scenario_manager.formula_engine, "calculate_scenario"
        ) as mock_calc:
    ...
    def test_compare_scenarios(self, scenario_manager, mock_db):
        ...
        result = scenario_manager.compare_scenarios(
            base_scenario_id=1, compare_scenario_id=2, user_id=1
        )
    ...
    def test_compare_scenarios_not_found(self, scenario_manager, mock_db):
        ...
        with pytest.raises(ValueError, match="One or both scenarios not found"):
            scenario_manager.compare_scenarios(
                base_scenario_id=999, compare_scenario_id=998, user_id=1
            )
...
class TestScenarioCloneResult:
    ...
    def test_scenario_clone_result_success(self):
        ...
        result = ScenarioCloneResult(
            original_scenario_id=1, new_scenario_id=2, parameters_copied=5, success=True
        )
