...
from app.tasks.file_processing import DatabaseTask, process_uploaded_file
...
class TestFileProcessingTasks:
    ...
    def test_process_uploaded_file_success(
        self, mock_validator, mock_parser, mock_file_service, mock_db, sample_file
    ):
        ...
        # Run the task
        result = process_uploaded_file.__wrapped__(
            mock_task, mock_db, 1, {"option": "test"}
        )
    ...
    @patch("app.tasks.file_processing.FileService")
    @patch("app.tasks.file_processing.ExcelParser")
    def test_process_uploaded_file_parsing_error(
        self, mock_parser, mock_file_service, mock_db, sample_file
    ):
        ...
        mock_parser_instance.parse_excel_file.side_effect = ParsingError(
            "Parsing failed"
        )
    ...
    @patch("app.tasks.file_processing.ExcelParser")
    @patch("app.tasks.file_processing.AdvancedValidator")
    def test_process_uploaded_file_validation_failed(
        self, mock_validator, mock_parser, mock_file_service, mock_db, sample_file
    ):
...
class TestScheduledTasks:
    ...
    def test_cleanup_expired_files(
        self, mock_file_service, mock_session_local, mock_alert
    ):
    ...
    @patch("app.tasks.scheduled_tasks.FileService")
    @patch("app.tasks.scheduled_tasks.SessionLocal")
    def test_cleanup_expired_files_error(
        self, mock_session_local, mock_file_service, mock_alert
    ):
        """Test cleanup task with error"""
        mock_session_local.return_value.__enter__.side_effect = Exception(
            "Database error"
        )
