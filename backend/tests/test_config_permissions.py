...
def test_permission_checker_basic():
    ...
    assert PermissionChecker.has_any_permission(
        viewer_roles, [Permission.USER_READ, Permission.DATA_EXPORT]
    )
    assert PermissionChecker.has_all_permissions(
        admin_roles, [Permission.USER_READ, Permission.USER_DELETE]
    )
    ...
    assert PermissionChecker.can_access_resource(
        admin_roles, 1, 2, Permission.MODEL_READ
    )
    assert PermissionChecker.can_access_resource(
        viewer_roles, 1, 1, Permission.MODEL_READ
    )
    assert PermissionChecker.can_access_resource(
        viewer_roles, 2, 1, Permission.MODEL_READ
    )
...
def test_can_access_resource_admin():
    ...
    assert PermissionChecker.can_access_resource(admin, 2, 1, Permission.MODEL_UPDATE)
