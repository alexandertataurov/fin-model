...
from app.models.mfa import OAuthAccount, WebAuthnCredential
...
class TestOAuthService:
    ...
    def test_generate_unique_username(self, test_db):
        ...
        try:
            ...
            # Create a user with the first username to test duplicate handling
            from app.core.security import get_password_hash
            existing_user = User(
                email="existing@example.com",
                username="test",  # This will conflict with the first username
                first_name="Existing",
                last_name="User",
            ...
            # Now test that duplicate usernames get numbered
            username2 = oauth_service.generate_unique_username("test@example.com")
            assert username2 == "test1"
            
            # Create another user with test1 username
            existing_user2 = User(
                email="existing2@example.com",
                username="test1",  # This will conflict with the second username
                first_name="Existing2",
                last_name="User",
                full_name="Existing2 User",
                hashed_password=get_password_hash("password"),
                is_verified=True,
                is_active=True
            )
            db_session.add(existing_user2)
            db_session.commit()
            
            # Test with another duplicate
            username3 = oauth_service.generate_unique_username("test@example.com")
            assert username3 == "test2"
            
        finally:
            db_session.close()

    def test_can_unlink_oauth(self, test_db):
...
class TestWebAuthnService:
    ...
    def test_cleanup_expired_challenges(self, test_db):
    ...
    def test_has_webauthn_credentials(self, test_db):
...
class TestAuthenticationFlow:
    ...
    @pytest.mark.asyncio
    async def test_complete_mfa_flow_simulation(self, test_db):
        ...
        try:
            ...
            new_token = totp.now()
            mfa_verified = mfa_service.verify_mfa_token(
                test_user, new_token, use_backup=False
            )
            assert mfa_verified
    ...
    @pytest.mark.asyncio
    async def test_oauth_user_creation(self, test_db):
