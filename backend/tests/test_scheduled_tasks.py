...
class DummyCleanupService:
    ...
    def get_cleanup_statistics(self):
        return {
            "reclaimable_storage_mb": 50,
            "eligible_for_cleanup": 5,
            "storage_used_mb": 500,
        }
...
def patch_deps(monkeypatch):
    ...
    sys.modules["app.services.cloud_storage"] = types.SimpleNamespace(
        CloudStorageManager=DummyCloud
    )
    sys.modules["app.services.virus_scanner"] = types.SimpleNamespace(
        VirusScanManager=DummyScanner
    )
    sys.modules["app.services.analytics_service"] = types.SimpleNamespace(
        AnalyticsService=DummyAnalytics
    )
    return dummy_send
