...
def test_create_template_commits():
    db = MagicMock()
    with patch("app.services.report_service.PDFReportGenerator"), patch(
        "app.services.report_service.ExcelExporter"
    ), patch("app.services.report_service.DashboardMetricsService"), patch(
        "app.services.report_service.ReportTemplate"
    ) as mock_model:
...
def test_export_chart_invalid_format():
    db = MagicMock()
    with patch("app.services.report_service.PDFReportGenerator") as pdf, patch(
        "app.services.report_service.ExcelExporter"
    ), patch("app.services.report_service.DashboardMetricsService"):
